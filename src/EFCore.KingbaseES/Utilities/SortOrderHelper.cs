...
internal static class SortOrderHelper {
  public static bool
  IsDefaultNullSortOrder(IReadOnlyList<NullSortOrder>? nullSortOrders,
    ...
    for (var i = 0; i < nullSortOrders.Count; i++) {
      ...
      // We need to consider the ASC/DESC sort order to determine the default
      // NULLS FIRST/LAST sort order.
      if (isDescendingValues is not null &&
          (isDescendingValues.Count == 0 || isDescendingValues[i])) {
