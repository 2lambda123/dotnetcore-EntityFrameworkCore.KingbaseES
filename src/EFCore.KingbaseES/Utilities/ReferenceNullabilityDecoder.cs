...
// Note: this is a very partial implementation that does not correctly decode
// all scenarios.
internal class ReferenceNullabilityDecoder {
  ...
  protected virtual bool IsNonNullableReferenceType(MemberInfo memberInfo) {
    ...
    // First check for [MaybeNull] on the return value. If it exists, the member
    // is nullable. Note: avoid using GetCustomAttribute<> below because of
    // https://github.com/mono/mono/issues/17477
    var isMaybeNull = memberInfo switch {
      FieldInfo f => f.CustomAttributes.Any(a => a.AttributeType ==
                                                 typeof(MaybeNullAttribute)),
      PropertyInfo p =>
          p.GetMethod?.ReturnParameter?.CustomAttributes?.Any(
              a => a.AttributeType == typeof(MaybeNullAttribute)) == true,
      _ => false
    };
    ...
    // For C# 8.0 nullable types, the C# compiler currently synthesizes a
    // NullableAttribute that expresses nullability into assemblies it produces.
    // If the model is spread across more than one assembly, there will be
    // multiple versions of this attribute, so look for it by name, caching to
    // avoid reflection on every check. Note that this may change - if
    // https://github.com/dotnet/corefx/issues/36222 is done we can remove all
    // of this.
    ...
    // No attribute on the member, try to find a NullableContextAttribute on the
    // declaring type
  ...
  internal bool IsArrayOrListElementNonNullable(MemberInfo memberInfo) {
    if (!memberInfo.GetMemberType().TryGetElementType(out var elementType)) {
      throw new ArgumentException(
          $"Member {memberInfo.DeclaringType?.Name}.{memberInfo.Name} isn't an array or generic List",
          nameof(memberInfo));
    }
    ...
    // First look for NullableAttribute on the member itself.
    // Either it has two flags - one for the array/list and one for the element
    // - or a single compressed byte since both have the same nullability.
    if (TryGetNullableFlags(memberInfo, out var flags)) {
  ...
  private bool TryGetNullableFlags(MemberInfo memberInfo,
                                   [NotNullWhen(true)] out byte[]? flags) {
    if (memberInfo.GetCustomAttributes().FirstOrDefault(
            a => a.GetType().FullName == NullableAttributeFullName)
  ...
  private bool IsContextNonNullable(Type type) {
    ...
    if (Attribute.GetCustomAttributes(type).FirstOrDefault(
      ...
      if (_cache.NullableContextFlagFieldInfo?.GetValue(contextAttr)
              is byte flag) {
