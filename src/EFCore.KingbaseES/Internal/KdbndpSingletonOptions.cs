...
public class KdbndpSingletonOptions : IKdbndpSingletonOptions {
  ...
  public virtual void Initialize(IDbContextOptions options) {
    var kdbndpOptions = options.FindExtension<KdbndpOptionsExtension>() ??
  ...
  public virtual void Validate(IDbContextOptions options) {
    ...
    if (PostgresVersion != kdbndpOptions.PostgresVersion) {
      throw new InvalidOperationException(CoreStrings.SingletonOptionChanged(
    ...
    if (kdbndpOptions.DataSource is not null &&
        !ReferenceEquals(DataSource, kdbndpOptions.DataSource)) {
      throw new InvalidOperationException(
          KdbndpStrings.TwoDataSourcesInSameServiceProvider(
              nameof(DbContextOptionsBuilder.UseInternalServiceProvider)));
    }

    if (!UserRangeDefinitions.SequenceEqual(
            kdbndpOptions.UserRangeDefinitions)) {
      throw new InvalidOperationException(CoreStrings.SingletonOptionChanged(
