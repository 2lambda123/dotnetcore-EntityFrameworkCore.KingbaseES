...
internal static class EnumerableMethods {
  ...
  // public static MethodInfo GroupByWithKeyElementResultSelectorAndComparer {
  // get; }
  ...
  // public static MethodInfo ToDictionaryWithKeyElementSelectorAndComparer {
  // get; }
  ...
  // private static Dictionary<Type, MethodInfo> SumWithoutSelectorMethods {
  // get; } private static Dictionary<Type, MethodInfo> SumWithSelectorMethods {
  // get; } private static Dictionary<Type, MethodInfo>
  // AverageWithoutSelectorMethods { get; } private static Dictionary<Type,
  // MethodInfo> AverageWithSelectorMethods { get; } private static
  // Dictionary<Type, MethodInfo> MaxWithoutSelectorMethods { get; } private
  // static Dictionary<Type, MethodInfo> MaxWithSelectorMethods { get; } private
  // static Dictionary<Type, MethodInfo> MinWithoutSelectorMethods { get; }
  ...
  static EnumerableMethods() {
    ...
    // GetMethod(nameof(Enumerable.DefaultIfEmpty), 1,
    //     types => new[] { typeof(IEnumerable<>).MakeGenericType(types[0]) });

    // DefaultIfEmptyWithArgument = GetMethod(nameof(Enumerable.DefaultIfEmpty),
    // 1,
    //     types => new[]
    //     {
    //         typeof(IEnumerable<>).MakeGenericType(types[0]),
    ...
    // Distinct = GetMethod(nameof(Enumerable.Distinct), 1, types => new[] {
    // typeof(IEnumerable<>).MakeGenericType(types[0]) });
    ...
    // GetMethod(nameof(Enumerable.FirstOrDefault), 1,
    //     types => new[] { typeof(IEnumerable<>).MakeGenericType(types[0]) });
    ...
    // GetMethod(nameof(Enumerable.GroupBy), 4,
    //     types => new[]
    //     {
    //         typeof(IEnumerable<>).MakeGenericType(types[0]),
    //         typeof(Func<,>).MakeGenericType(types[0], types[1]),
    ...
    //             types[1], typeof(IEnumerable<>).MakeGenericType(types[2]),
    //             types[3])
    //     });
    ...
    //         typeof(Func<,>).MakeGenericType(types[0], types[1]),
    //         typeof(Func<,,>).MakeGenericType(
    //             types[1], typeof(IEnumerable<>).MakeGenericType(types[0]),
    //             types[2])
    //     });
    ...
    // GetMethod(nameof(Enumerable.LastOrDefault), 1,
    //     types => new[] { typeof(IEnumerable<>).MakeGenericType(types[0]) });

    // LastOrDefaultWithPredicate = GetMethod(nameof(Enumerable.LastOrDefault),
    // 1,
    //     types => new[]
    //     {
    //         typeof(IEnumerable<>).MakeGenericType(types[0]),
    ...
    // MaxWithoutSelector = GetMethod(nameof(Enumerable.Max), 1, types => new[]
    // { typeof(IEnumerable<>).MakeGenericType(types[0]) });
    ...
    // MinWithoutSelector = GetMethod(nameof(Enumerable.Min), 1, types => new[]
    // { typeof(IEnumerable<>).MakeGenericType(types[0]) });
    ...
    // OfType = GetMethod(nameof(Enumerable.OfType), 1, _ => new[] {
    // typeof(IEnumerable) });
    ...
    // Reverse = GetMethod(nameof(Enumerable.Reverse), 1, types => new[] {
    // typeof(IEnumerable<>).MakeGenericType(types[0]) });
    ...
    // GetMethod(nameof(Enumerable.SelectMany), 2,
    //     types => new[]
    //     {
    //         typeof(IEnumerable<>).MakeGenericType(types[0]),
    //         typeof(Func<,>).MakeGenericType(
    ...
    // GetMethod(nameof(Enumerable.SelectMany), 3,
    //     types => new[]
    //     {
    //         typeof(IEnumerable<>).MakeGenericType(types[0]),
    //         typeof(Func<,>).MakeGenericType(
    ...
    // GetMethod(nameof(Enumerable.SingleOrDefault), 1,
    //     types => new[] { typeof(IEnumerable<>).MakeGenericType(types[0]) });
    ...
    // ToArray = GetMethod(nameof(Enumerable.ToArray), 1, types => new[] {
    // typeof(IEnumerable<>).MakeGenericType(types[0]) });

    // ToList = GetMethod(nameof(Enumerable.ToList), 1, types => new[] {
    // typeof(IEnumerable<>).MakeGenericType(types[0]) });
    ...
    //         nameof(Enumerable.Average), 0, _ => new[] {
    ...
    MethodInfo GetMethod(string name, int genericParameterCount,
        ...
        queryableMethodGroups[name].Single(
            mi => ((genericParameterCount == 0 && !mi.IsGenericMethod) ||
                  ...
                  mi.GetParameters()
                      ...
                      .SequenceEqual(parameterGenerator(
