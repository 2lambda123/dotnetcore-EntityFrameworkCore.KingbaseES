...
public class PostgresCollation {
  ...
  internal PostgresCollation(IReadOnlyAnnotatable annotatable,
  ...
  GetOrAddCollation(IMutableAnnotatable annotatable, string? schema,
                    string name, string lcCollate, string lcCtype,
    ...
    return new PostgresCollation(annotatable, annotationName) {
      LcCollate = lcCollate, LcCtype = lcCtype, Provider = provider,
  ...
  public static PostgresCollation? FindCollation(
    ...
    return annotatable[annotationName] is null
               ? null
               : new PostgresCollation(annotatable, annotationName);
  }
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  public static IEnumerable<PostgresCollation>
  GetCollations(IReadOnlyAnnotatable annotatable) =>
      Check.NotNull(annotatable, nameof(annotatable))
          .GetAnnotations()
          .Where(a => a.Name.StartsWith(
                     KdbndpAnnotationNames.CollationDefinitionPrefix,
  ...
  private (string? Schema, string? Name, string? LcCollate, string? LcCtype,
           string? Provider, bool? IsDeterministic)
      GetData() => Deserialize(Annotatable.FindAnnotation(_annotationName));

  private void
  SetData(string? lcCollate = null, string? lcCtype = null,
          string? provider = null,
          bool? deterministic = null) => Annotatable[_annotationName] =
      $"{lcCollate ?? LcCollate},{lcCtype ?? LcCtype},{provider ?? Provider},{deterministic ?? IsDeterministic}";

  private static (string? Schema, string? Name, string? LcCollate,
                  string? LcCtype, string? Provider, bool? IsDeterministic)
    ...
    if (elements.Length != 4) {
      throw new ArgumentException(
          $"Cannot parse collation annotation value: {value}");
    }
    ...
    var isDeterministic = elements[3] is {}
    isDeterminsticString ? bool.Parse(isDeterminsticString) : (bool?)null;
    ...
    // schema/collation name, let somebody complain first.
    var schemaAndName =
        annotation.Name
            .Substring(KdbndpAnnotationNames.CollationDefinitionPrefix.Length)
            .Split('.');
    ...
    case 2:
      return (schemaAndName[0], schemaAndName[1], elements[0], elements[1],
              elements[2], isDeterministic);
    default:
      throw new ArgumentException(
          $"Cannot parse collation name from annotation: {annotation.Name}");
    }
  }
}
