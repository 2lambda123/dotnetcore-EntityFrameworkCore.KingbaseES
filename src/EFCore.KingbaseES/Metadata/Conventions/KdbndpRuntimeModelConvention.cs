...
public class KdbndpRuntimeModelConvention : RelationalRuntimeModelConvention {
  ...
  ProcessModelAnnotations(Dictionary<string, object?> annotations, IModel model,
                          RuntimeModel runtimeModel, bool runtime) {
...
#pragma warning restore CS0618
      ...
      foreach (var annotationName in annotations.Keys.Where(
                   k => k.StartsWith(
                        ...
                        k.StartsWith(KdbndpAnnotationNames.EnumPrefix,
                                     StringComparison.Ordinal) ||
                        k.StartsWith(KdbndpAnnotationNames.RangePrefix,
                                     StringComparison.Ordinal))) {
  ...
  protected override void ProcessEntityTypeAnnotations(
      Dictionary<string, object?> annotations, IEntityType entityType,
      RuntimeEntityType runtimeEntityType, bool runtime) {
    base.ProcessEntityTypeAnnotations(annotations, entityType,
                                      runtimeEntityType, runtime);
  ...
  /// <inheritdoc />
  protected override void
  ProcessPropertyAnnotations(Dictionary<string, object?> annotations,
                             IProperty property,
                             RuntimeProperty runtimeProperty, bool runtime) {
    base.ProcessPropertyAnnotations(annotations, property, runtimeProperty,
                                    runtime);
    ...
    if (!runtime) {
      ...
      if (!annotations.ContainsKey(
        ...
        annotations[KdbndpAnnotationNames.ValueGenerationStrategy] =
  ...
  /// <inheritdoc />
  protected override void
  ProcessIndexAnnotations(Dictionary<string, object?> annotations, IIndex index,
                          RuntimeIndex runtimeIndex, bool runtime) {
