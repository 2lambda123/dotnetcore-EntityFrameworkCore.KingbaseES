...
public class CockroachDbInterleaveInParent {
  private const string AnnotationName =
  ...
  private (string? ParentTableSchema, string ParentTableName,
           List<string> InterleavePrefix) GetData() {
    var str = Annotatable[AnnotationName] as string;
    return str is null ? (null, null!, null!) : Deserialize(str);
  }

  private void
  SetData(string? parentTableSchema, string parentTableName,
          List<string> interleavePrefix) => Annotatable[AnnotationName] =
      Serialize(parentTableSchema, parentTableName, interleavePrefix);

  private static string Serialize(string? parentTableSchema,
  ...
  internal static (string? ParentTableSchema, string ParentTableName,
                   List<string> InterleavePrefix) Deserialize(string value) {
    ...
    try {
      ...
      throw new ArgumentException(
          $"Couldn't deserialize {nameof(CockroachDbInterleaveInParent)} from annotation",
          ex);
    }
  }
  ...
  private static string? ExtractValue(string value, ref int position) {
    ...
    var extracted =
        value.Substring(position, end - position).Replace("''", "'");
    position = end + 1;
