...
public class KdbndpAnnotationProvider : RelationalAnnotationProvider {
  ...
  /// </summary>
  public KdbndpAnnotationProvider(
  ...
  public override IEnumerable<IAnnotation> For(ITable table, bool designTime) {
    ...
    // Model validation ensures that these facets are the same on all mapped
    // entity types
    var entityType = (IEntityType)table.EntityTypeMappings.First().TypeBase;

    if (entityType.GetIsUnlogged()) {
      yield return new Annotation(KdbndpAnnotationNames.UnloggedTable,
                                  entityType.GetIsUnlogged());
    }
    ...
    foreach (var storageParamAnnotation in entityType.GetAnnotations().Where(
                 a => a.Name.StartsWith(
  ...
  public override IEnumerable<IAnnotation> For(IColumn column,
    ...
    var valueGeneratedProperty =
        column.PropertyMappings
            ...
            .Select(m => m.Property)
            .FirstOrDefault(p => p.GetValueGenerationStrategy(table) switch {
    ...
    if (valueGeneratedProperty is not null) {
      var valueGenerationStrategy =
          valueGeneratedProperty.GetValueGenerationStrategy();
      yield return new Annotation(KdbndpAnnotationNames.ValueGenerationStrategy,
                                  valueGenerationStrategy);

      if (valueGenerationStrategy is KdbndpValueGenerationStrategy
              .IdentityByDefaultColumn or
                  KdbndpValueGenerationStrategy.IdentityAlwaysColumn) {
        if (valueGeneratedProperty[KdbndpAnnotationNames.IdentityOptions] is
                string identityOptions) {
          yield return new Annotation(KdbndpAnnotationNames.IdentityOptions,
                                      identityOptions);
        }
      }
    }

    // If the property has a collation explicitly defined on it via the standard
    // EF mechanism, it will get passed on the Collation property (we don't need
    // to do anything). Otherwise, if a model-wide default column collation
    // exists, pass that through our custom annotation. Note that this mechanism
    // is obsolete, and EF Core's bulk model configuration can be used instead;
    // but we continue to support it for backwards compat.
#pragma warning disable CS0618
    if (column.PropertyMappings.All(m => m.Property.GetCollation() is null) &&
      ...
      yield return new Annotation(KdbndpAnnotationNames.DefaultColumnCollation,
...
#pragma warning restore CS0618
    ...
    if (column.PropertyMappings.Select(m => m.Property.GetTsVectorConfig())
      ...
      yield return new Annotation(KdbndpAnnotationNames.TsVectorConfig,
                                  tsVectorConfig);
    }

    valueGeneratedProperty =
        column.PropertyMappings.Select(m => m.Property)
    ...
    if (valueGeneratedProperty is not null) {
      ...
      yield return new Annotation(
          ...
          valueGeneratedProperty.GetTsVectorProperties()!
              .Select(p2 =>
                          valueGeneratedProperty.DeclaringType.FindProperty(p2)!
                              .GetColumnName(tableIdentifier))
              .ToArray());
    }

    // JSON columns have no property mappings so all annotations that rely on
    // property mappings should be skipped for them
    if (column is not JsonColumn && column.PropertyMappings.FirstOrDefault()
                                        ?.Property.GetCompressionMethod()
                                            is {} compressionMethod) {
      // Model validation ensures that these facets are the same on all mapped
      // properties
      yield return new Annotation(KdbndpAnnotationNames.CompressionMethod,
                                  compressionMethod);
    }
  }
  ...
  public override IEnumerable<IAnnotation> For(ITableIndex index,
    ...
    if (modelIndex.GetCollation() is {} collation) {
      yield return new Annotation(RelationalAnnotationNames.Collation,
                                  collation);
    }
    ...
    if (modelIndex.GetOperators() is {} operators) {
      yield return new Annotation(KdbndpAnnotationNames.IndexOperators,
                                  operators);
    }

    if (modelIndex.GetNullSortOrder() is {} nullSortOrder) {
      yield return new Annotation(KdbndpAnnotationNames.IndexNullSortOrder,
                                  nullSortOrder);
    }
    ...
    if (modelIndex.GetIncludeProperties() is {} includeProperties) {
      var tableIdentifier =
          StoreObjectIdentifier.Table(index.Table.Name, index.Table.Schema);
    ...
    if (modelIndex.IsCreatedConcurrently() is {} isCreatedConcurrently) {
      yield return new Annotation(KdbndpAnnotationNames.CreatedConcurrently,
                                  isCreatedConcurrently);
    }

    if (modelIndex.GetAreNullsDistinct() is {} nullsDistinct) {
      yield return new Annotation(KdbndpAnnotationNames.NullsDistinct,
                                  nullsDistinct);
    }

    foreach (var storageParamAnnotation in modelIndex.GetAnnotations().Where(
                 a => a.Name.StartsWith(
                     KdbndpAnnotationNames.StorageParameterPrefix,
                     StringComparison.Ordinal))) {
      yield return storageParamAnnotation;
    ...
    if (modelIndex[KdbndpAnnotationNames.IndexSortOrder] is
            IReadOnlyList<SortOrder> legacySortOrder) {
      yield return new Annotation(KdbndpAnnotationNames.IndexSortOrder,
                                  legacySortOrder);
    }
  }
  ...
  public override IEnumerable<IAnnotation> For(IRelationalModel model,
    ...
    return model.Model.GetAnnotations().Where(
        a => a.Name.StartsWith(KdbndpAnnotationNames.PostgresExtensionPrefix,
                               StringComparison.Ordinal) ||
             a.Name.StartsWith(KdbndpAnnotationNames.EnumPrefix,
                               StringComparison.Ordinal) ||
             a.Name.StartsWith(KdbndpAnnotationNames.RangePrefix,
                               StringComparison.Ordinal) ||
             a.Name.StartsWith(KdbndpAnnotationNames.CollationDefinitionPrefix,
                               StringComparison.Ordinal));
  }
}
