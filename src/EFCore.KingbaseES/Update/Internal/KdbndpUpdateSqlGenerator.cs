...
public class KdbndpUpdateSqlGenerator : UpdateSqlGenerator {
  ...
  public override ResultSetMapping AppendInsertOperation(
      StringBuilder commandStringBuilder, IReadOnlyModificationCommand command,
      int commandPosition, out bool requiresTransaction) =>
      AppendInsertOperation(commandStringBuilder, command, commandPosition,
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  public virtual ResultSetMapping
  AppendInsertOperation(StringBuilder commandStringBuilder,
                        IReadOnlyModificationCommand command,
                        int commandPosition, bool overridingSystemValue,
    ...
    AppendInsertCommand(commandStringBuilder, name, schema, writeOperations,
                        readOperations, overridingSystemValue);

    requiresTransaction = false;

    return readOperations.Count > 0 ? ResultSetMapping.LastInResultSet
  ...
  AppendInsertCommand(StringBuilder commandStringBuilder, string name,
                      string? schema,
                      IReadOnlyList<IColumnModification> writeOperations,
                      IReadOnlyList<IColumnModification> readOperations,
                      bool overridingSystemValue) {
    AppendInsertCommandHeader(commandStringBuilder, name, schema,
                              writeOperations);
  ...
  public override ResultSetMapping AppendUpdateOperation(
      StringBuilder commandStringBuilder, IReadOnlyModificationCommand command,
      int commandPosition, out bool requiresTransaction) {
    // The default implementation adds RETURNING 1 to do concurrency check (was
    // the row actually updated), but in KingbaseES we check the per-statement
    // row-affected value exposed by Kdbndp in the batch; so no need for
    // RETURNING 1.
    var name = command.TableName;
    var schema = command.Schema;
    var operations = command.ColumnModifications;
    ...
    AppendUpdateCommand(commandStringBuilder, name, schema, writeOperations,
  ...
  AppendUpdateColumnValue(ISqlGenerationHelper updateSqlGeneratorHelper,
                          IColumnModification columnModification,
                          StringBuilder stringBuilder, string name,
                          string? schema) {
    if (columnModification.JsonPath is not(null or "$")) {
      ...
      Check.DebugAssert(columnModification.TypeMapping.StoreType is "jsonb",
                        "Non-jsonb type mapping in JSON partial update");

      // TODO: Lax or not?
      stringBuilder.Append("jsonb_set(")
          .Append(updateSqlGeneratorHelper.DelimitIdentifier(
              columnModification.ColumnName))
          .Append(", '{");

      // TODO: Unfortunately JsonPath is provided as a JSONPATH string, but PG's
      // jsonb_set requires the path as an array. Parse the components back out
      // (https://github.com/dotnet/efcore/issues/32185)
      ...
      base.AppendUpdateColumnValue(updateSqlGeneratorHelper, columnModification,
                                   stringBuilder, name, schema);
  ...
  public override ResultSetMapping AppendDeleteOperation(
      ...
      int commandPosition, out bool requiresTransaction) {
    // The default implementation adds RETURNING 1 to do concurrency check (was
    // the row actually deleted), but in KingbaseES we check the per-statement
    // row-affected value exposed by Kdbndp in the batch; so no need for
    // RETURNING 1.
    var name = command.TableName;
    var schema = command.Schema;
    ...
    AppendDeleteCommand(commandStringBuilder, name, schema,
  ...
  public override ResultSetMapping AppendStoredProcedureCall(
      StringBuilder commandStringBuilder, IReadOnlyModificationCommand command,
      int commandPosition, out bool requiresTransaction) {
    Check.DebugAssert(command.StoreStoredProcedure is not null,
    ...
    // KingbaseES supports neither a return value nor a result set with stored
    // procedures, only output parameters.
    Check.DebugAssert(storedProcedure.ReturnValue is null,
                      "storedProcedure.Return is null");
    Check.DebugAssert(!storedProcedure.ResultColumns.Any(),
                      "!storedProcedure.ResultColumns.Any()");

    SqlGenerationHelper.DelimitIdentifier(
        commandStringBuilder, storedProcedure.Name, storedProcedure.Schema);

    commandStringBuilder.Append('(');

    var first = true;

    // Only positional parameter style supported for now, see
    // https://github.com/dotnet/efcore/issues/28439

    // Note: the column modifications are already ordered according to the sproc
    // parameter ordering (see ModificationCommand.GenerateColumnModifications)
    for (var i = 0; i < command.ColumnModifications.Count; i++) {
      var columnModification = command.ColumnModifications[i];
      var parameter =
          (IStoreStoredProcedureParameter)columnModification.Column!;
      ...
      // very different from SQL Server, where output parameter values can be
      // sent back in addition to result sets.
      if (parameter.Direction.HasFlag(ParameterDirection.Output)) {
        // The distinction between having only a rows affected output parameter
        // and having other non-rows affected parameters is important later on
        // (i.e. whether we need to propagate or not).
        resultSetMapping =
            parameter == command.RowsAffectedColumn &&
  ...
  AppendObtainNextSequenceValueOperation(StringBuilder commandStringBuilder,
                                         string name, string? schema) {
    commandStringBuilder.Append("nextval('");
    SqlGenerationHelper.DelimitIdentifier(
        commandStringBuilder, Check.NotNull(name, nameof(name)), schema);
    commandStringBuilder.Append("')");
  }
}
