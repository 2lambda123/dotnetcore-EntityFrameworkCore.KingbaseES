...
public class KdbndpModificationCommand : ModificationCommand {
  ...
  /// </summary>
  public KdbndpModificationCommand(
    ...
    _detailedErrorsEnabled =
        modificationCommandParameters.DetailedErrorsEnabled;
  }
  ...
  ProcessSinglePropertyJsonUpdate(ref ColumnModificationParameters parameters) {
    // PG jsonb_set accepts a jsonb parameter for the value to be set - not an
    // int, boolean or string like many other providers. So we always pass the
    // value through the mapping's ToJsonString() (except for null).
  ...
  public override void PropagateResults(RelationalDataReader relationalReader) {
    // The default implementation of PropagateResults skips (output) parameters,
    // since for e.g. SQL Server these aren't yet populated when consuming the
    // result set (propagating output columns is done later, after the reader is
    // closed). However, in KingbaseES, output parameters actually get returned
    // as the result set, so we override and take care of that here.
    ...
    // For regular result sets, results are always propagated back into entity
    // properties. But with stored procedures, there may be a rows affected
    // result column (generated by an output parameter definition). Skip these.
    ...
    columnModification.Value = columnModification.Property.GetReaderFieldValue(
        relationalReader, readerIndex, _detailedErrorsEnabled);
  }
}
}
