...
// Migrator is EF-pubternal, but overriding it is the only way to force Kdbndp
// to ReloadTypes() after executing a migration which adds types to the database
...
public class KdbndpMigrator : Migrator {
  ...
  /// </summary>
  public KdbndpMigrator(IMigrationsAssembly migrationsAssembly,
      ...
      : base(migrationsAssembly, historyRepository, databaseCreator,
             ...
             migrationCommandExecutor, connection, sqlGenerationHelper,
             currentContext, modelRuntimeInitializer, logger, commandLogger,
             databaseProvider) {
  ...
  public override void Migrate(string? targetMigration = null) {
    ...
    PopulateMigrations(appliedMigrations.Select(t => t.MigrationId),
                       targetMigration, out var migrationsToApply,
                       out var migrationsToRevert, out _);
    ...
    var migrations =
        migrationsToApply.Count > 0 ? migrationsToApply : migrationsToRevert;
    var reloadTypes = migrations.SelectMany(m => m.UpOperations)
                          .OfType<AlterDatabaseOperation>()
                          .Any(o => o.GetPostgresExtensions().Any() ||
  ...
  /// </summary>
  public override
      async Task MigrateAsync(string? targetMigration = null,
