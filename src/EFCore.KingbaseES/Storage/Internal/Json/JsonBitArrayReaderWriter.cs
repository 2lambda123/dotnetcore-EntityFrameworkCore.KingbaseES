...
public sealed class JsonBitArrayReaderWriter : JsonValueReaderWriter<BitArray> {
  ...
  /// <inheritdoc />
  public override BitArray FromJsonTyped(ref Utf8JsonReaderManager manager,
    ...
    for (var i = 0; i < s.Length; i++) {
      bitArray[i] = s[i] switch {
        ...
        _ => throw new InvalidOperationException(
            $"Invalid character '{s[i]}' in BitArray string representation in JSON")
      };
    }
  ...
  public override void ToJsonTyped(Utf8JsonWriter writer, BitArray value) =>
      writer.WriteStringValue(string.Create(value.Length, value, (s, a) => {
        for (var i = 0; i < s.Length; i++) {
          s[i] = a[i] ? '1' : '0';
        }
      }));
