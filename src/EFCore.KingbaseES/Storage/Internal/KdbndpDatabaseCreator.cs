...
public class KdbndpDatabaseCreator : RelationalDatabaseCreator {
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  public virtual TimeSpan RetryDelay {
    get; set;
    ...
    using (var masterConnection = _connection.CreateAdminConnection()) {
      try {
        Dependencies.MigrationCommandExecutor.ExecuteNonQuery(
            CreateCreateOperations(), masterConnection);
      } catch (KdbndpException e) when (e is { SqlState : "23505" }) {
        // This occurs when two connections are trying to create the same
        // database concurrently (happens in the tests). Simply ignore the
        // error.
      }
  ...
  /// </summary>
  public override
      async Task CreateAsync(CancellationToken cancellationToken = default) {
    var masterConnection = _connection.CreateAdminConnection();
    await using (masterConnection.ConfigureAwait(false)) {
      try {
        await Dependencies.MigrationCommandExecutor
            .ExecuteNonQueryAsync(CreateCreateOperations(), masterConnection,
                                  cancellationToken)
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  public override bool HasTables() => Dependencies.ExecutionStrategy.Execute(
      _connection,
      connection => (bool)CreateHasTablesCommand().ExecuteScalar(
          new RelationalCommandParameterObject(
              connection, null, null, Dependencies.CurrentContext.Context,
  ...
  HasTablesAsync(CancellationToken cancellationToken = default) =>
      Dependencies.ExecutionStrategy.ExecuteAsync(
          _connection,
          async (connection,
                 ct) => (bool)(await CreateHasTablesCommand()
                                   .ExecuteScalarAsync(
                                       new RelationalCommandParameterObject(
    ...
    return Dependencies.MigrationsSqlGenerator.Generate(
        new[] { new KdbndpCreateDatabaseOperation {
    ...
    {
    // When checking whether a database exists, pooling must be off, otherwise
    // we may attempt to reuse a pooled connection, which may be broken (this
    // happened in the tests). If Pooling is off, but Multiplexing is on -
    // KdbndpConnectionStringBuilder.Validate will throw, so we turn off
    // Multiplexing as well.
    var unpooledCsb = new KdbndpConnectionStringBuilder(
        _connection.ConnectionString) { Pooling = false, Multiplexing = false };

    using var _ = new TransactionScope(TransactionScopeOption.Suppress,
                                       TransactionScopeAsyncFlowOption.Enabled);
    var unpooledRelationalConnection =
        _connection.CloneWith(unpooledCsb.ToString());
    try {
      if (async) {
        await unpooledRelationalConnection
            .OpenAsync(errorsExpected: true,
      ...
      throw;
        ...
        ae.InnerExceptions.Any(ie => ie is KdbndpException pe &&
                                     IsDoesNotExist(pe))) {
      return false;
    ...
    using (var masterConnection = _connection.CreateAdminConnection()) {
      Dependencies.MigrationCommandExecutor.ExecuteNonQuery(
          CreateDropCommands(), masterConnection);
    }
    }
    ...
    await using (masterConnection) {
      await Dependencies.MigrationCommandExecutor
          .ExecuteNonQueryAsync(CreateDropCommands(), masterConnection,
                                cancellationToken)
          .ConfigureAwait(false);
    }
    }
    ...
    var operations = Dependencies.ModelDiffer.GetDifferences(
        null, designTimeModel.GetRelationalModel());
    var commands = Dependencies.MigrationsSqlGenerator.Generate(
        operations, designTimeModel);

    // If a KingbaseES extension, enum or range was added, we want Kdbndp to
    // reload all types at the ADO.NET level.
    var reloadTypes = operations.OfType<AlterDatabaseOperation>().Any(
        o => o.GetPostgresExtensions().Any() || o.GetPostgresEnums().Any() ||
             o.GetPostgresRanges().Any());

    try {
      Dependencies.MigrationCommandExecutor.ExecuteNonQuery(commands,
                                                            _connection);
    } catch (KdbndpException e) when (e is { SqlState : "23505" }) {
      // This occurs when two connections are trying to create the same database
      // concurrently (happens in the tests). Simply ignore the error.
    }

    if (reloadTypes &&
        _connection.DbConnection is KdbndpConnection KdbndpConnection) {
    ...
    try {
      await Dependencies.MigrationCommandExecutor
          .ExecuteNonQueryAsync(commands, _connection, cancellationToken)
          .ConfigureAwait(false);
    } catch (KdbndpException e) when (e is { SqlState : "23505" }) {
      // This occurs when two connections are trying to create the same database
      // concurrently (happens in the tests). Simply ignore the error.
    ...
    var operations = new MigrationOperation[] {
      ...
      // Issue #775
      new KdbndpDropDatabaseOperation { Name =
