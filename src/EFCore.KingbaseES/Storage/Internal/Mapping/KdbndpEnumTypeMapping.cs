...
public class KdbndpEnumTypeMapping : RelationalTypeMapping {
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  public KdbndpEnumTypeMapping(string storeType, Type enumType,
                               IKdbndpNameTranslator? nameTranslator = null)
      : base(storeType, enumType,
             jsonValueReaderWriter: (JsonValueReaderWriter?)
                 Activator.CreateInstance(typeof(JsonPgEnumReaderWriter<>)
                                              .MakeGenericType(enumType))) {
    if (!enumType.IsEnum || !enumType.IsValueType) {
      throw new ArgumentException(
          $"Enum type mappings require a CLR enum. {enumType.FullName} is not an enum.");
    }
...
#pragma warning restore CS0618
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  protected KdbndpEnumTypeMapping(RelationalTypeMappingParameters parameters,
                                  IKdbndpNameTranslator nameTranslator)
      : base(parameters) {
    NameTranslator = nameTranslator;
    _members =
        CreateValueMapping(parameters.CoreParameters.ClrType, nameTranslator);
  }
  ...
  private KdbndpEnumTypeMapping() : base("some_enum", typeof(int)) {
#pragma warning disable CS0618 // KdbndpConnection.GlobalTypeMapper is obsolete
    NameTranslator = KdbndpConnection.GlobalTypeMapper.DefaultNameTranslator;
#pragma warning restore CS0618
    _members = null!;
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  private static Dictionary<object, string>
  CreateValueMapping(Type enumType, IKdbndpNameTranslator nameTranslator) =>
      enumType.GetFields(BindingFlags.Static | BindingFlags.Public)
          .ToDictionary(x => x.GetValue(null)!,
                        x => x.GetCustomAttribute<KbNameAttribute>()?.KbName ??
  ...
  public sealed class JsonPgEnumReaderWriter<T> : JsonValueReaderWriter<T>
    ...
    ///     when updating to a new Entity Framework Core release.
    /// </summary>
    public override T FromJsonTyped(ref Utf8JsonReaderManager manager,
