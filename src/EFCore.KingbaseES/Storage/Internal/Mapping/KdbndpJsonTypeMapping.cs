...
/// <summary>
///     Supports the older Kdbndp-specific JSON mapping, allowing mapping
///     json/jsonb to text, to e.g. <see cref="JsonElement" /> (weakly-typed
///     mapping) or to arbitrary POCOs (but without them being modeled). For the
///     standard EF JSON support, which relies on owned entity modeling, see
///     <see cref="KdbndpOwnedJsonTypeMapping" />.
/// </summary>
public class KdbndpJsonTypeMapping : KdbndpTypeMapping {
  ...
  public KdbndpJsonTypeMapping(string storeType, Type clrType)
      : base(storeType, clrType,
             storeType == "jsonb" ? KdbndpDbType.Jsonb : KdbndpDbType.Json) {
    if (storeType != "json" && storeType != "jsonb") {
      throw new ArgumentException(
          $"{nameof(storeType)} must be 'json' or 'jsonb'", nameof(storeType));
    }
  }
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  protected KdbndpJsonTypeMapping(RelationalTypeMappingParameters parameters,
  ...
  EscapeSqlLiteral(string literal) => Check.NotNull(literal, nameof(literal))
                                          .Replace("'", "''");
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  public override Expression GenerateCodeLiteral(object value) => value switch {
    JsonDocument document => Expression.Call(
        ParseMethod, Expression.Constant(document.RootElement.ToString()),
  ...
  private static readonly Expression DefaultJsonDocumentOptions =
      Expression.New(typeof(JsonDocumentOptions));

  private static readonly MethodInfo ParseMethod =
      typeof(JsonDocument)
          .GetMethod(nameof(JsonDocument.Parse),
