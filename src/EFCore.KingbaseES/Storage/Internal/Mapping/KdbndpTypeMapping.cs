...
/// <summary>
///     The base class for mapping Kdbndp-specific types. It configures
///     parameters with the <see cref="KdbndpDbType" /> provider-specific type
///     enum.
/// </summary>
public abstract class KdbndpTypeMapping : RelationalTypeMapping,
                                          IKdbndpTypeMapping {
  /// <inheritdoc />
  ...
  /// <param name="jsonValueReaderWriter">Handles reading and writing JSON
  /// values for instances of the mapped type.</param>
  public KdbndpTypeMapping(string storeType, Type clrType,
  ...
  protected KdbndpTypeMapping(RelationalTypeMappingParameters parameters,
                              KdbndpDbType kdbndpDbType)
      : base(parameters) {
    KdbndpDbType = kdbndpDbType;
  }
  ...
  /// <summary>
  ///     Generates the SQL representation of a literal value meant to be
  ///     embedded in another literal value, e.g. in a range.
  /// </summary>
  ...
  /// <summary>
  ///     Generates the SQL representation of a literal value without
  ///     conversion, meant to be embedded in another literal value, e.g. in a
  ///     range.
  /// </summary>
  /// <param name="value">The literal value.</param>
  /// <returns>
  ///     The generated string.
  /// </returns>
  public virtual string GenerateEmbeddedProviderValueSqlLiteral(
      object? value) => value == null
                            ? "NULL"
                            : GenerateEmbeddedNonNullSqlLiteral(value);

  /// <summary>
  ///     Generates the SQL representation of a non-null literal value, meant to
  ///     be embedded in another literal value, e.g. in a range.
  /// </summary>
  /// <param name="value">The literal value.</param>
  /// <returns>
  ///     The generated string.
  /// </returns>
  protected virtual string GenerateEmbeddedNonNullSqlLiteral(
      object value) => GenerateNonNullSqlLiteral(value);

  // Copied from RelationalTypeMapping
  private object? ConvertUnderlyingEnumValueToEnum(object? value) =>
      value?.GetType().IsInteger() == true && ClrType.UnwrapNullableType()
                                                  .IsEnum
