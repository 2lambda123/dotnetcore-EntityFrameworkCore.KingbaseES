...
///     See: https://www.KingbaseES.org/docs/current/static/arrays.html
/// </remarks>
public class KdbndpArrayTypeMapping<TCollection, TConcreteCollection, TElement>
    : KdbndpArrayTypeMapping {
  /// <summary>
  ///     This is an internal API that supports the Entity Framework Core
  ///     infrastructure and not subject to the same compatibility standards as
  ///     public APIs. It may be changed or removed without notice in any
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  public static KdbndpArrayTypeMapping<TCollection, TConcreteCollection,
                                       TElement> Default {
    get;
  } = new();
  ...
  /// </summary>
  [UsedImplicitly]
  public KdbndpArrayTypeMapping(string storeType,
                                RelationalTypeMapping elementTypeMapping)
      : this(CreateParameters(storeType, elementTypeMapping)) {
    Check.DebugAssert(
        storeType.EndsWith("[]", StringComparison.Ordinal),
        "KdbndpArrayTypeMapping created for a non-array store type");
  }

  private static RelationalTypeMappingParameters
  CreateParameters(string storeType, RelationalTypeMapping elementMapping) {
    ValueConverter? converter = null;

    // We do GetElementType for multidimensional arrays - these don't implement
    // generic IEnumerable<>
    var elementType =
        typeof(TCollection).TryGetElementType(typeof(IEnumerable<>)) ??
        typeof(TCollection).GetElementType();

    Check.DebugAssert(elementType is not null,
                      "modelElementType cannot be null");

    if (elementMapping.Converter is {} elementConverter) {
      var providerElementType = elementConverter.ProviderClrType;

      // Nullability has been unwrapped on the element converter's provider CLR
      // type, so add it back here if needed
      ...
      converter = (ValueConverter)Activator.CreateInstance(
          typeof(KdbndpArrayConverter<,, >)
              .MakeGenericType(typeof(TCollection), typeof(TConcreteCollection),
...
#pragma warning restore EF1001
    ...
    if (elementJsonReaderWriter is not null &&
        elementJsonReaderWriter.ValueType !=
            typeof(TElement).UnwrapNullableType()) {
    ...
    // If there's no JsonValueReaderWriter on the element, we also don't set one
    // on its array (this is for rare edge cases such as
    // KdbndpRowValueTypeMapping).
    // TODO: Also, we don't (yet) support JSON serialization of multidimensional
    // arrays.
    var collectionJsonReaderWriter =
        elementJsonReaderWriter is null ||
                typeof(TCollection).IsArray &&
                    typeof(TCollection).GetArrayRank() > 1
            ? null
            : (JsonValueReaderWriter?)Activator.CreateInstance(
                  (elementType.IsNullableValueType()
                      ...
                      .MakeGenericType(typeof(TCollection),
    ...
    return new RelationalTypeMappingParameters(
        new CoreTypeMappingParameters(
            typeof(TCollection), converter, comparer,
  ...
  protected KdbndpArrayTypeMapping(RelationalTypeMappingParameters parameters)
    ...
    if (clrType.TryGetElementType(typeof(IEnumerable<>)) == null &&
        clrType.GetElementType() == null) {
      throw new ArgumentException(
          $"CLR type '{parameters.CoreParameters.ClrType}' isn't an IEnumerable");
    }

    // If the element mapping has an KdbndpDbType or DbType, set our own
    // KdbndpDbType as an array of that. Otherwise let the ADO.NET layer infer
    // the KingbaseES type. We can't always let it infer, otherwise when given a
    // byte[] it will infer byte (but we want smallint[])
    KdbndpDbType =
        KdbndpTypes.KdbndpDbType.Array |
        (ElementTypeMapping is IKdbndpTypeMapping elementKdbndpTypeMapping
         ...
         : ElementTypeMapping.DbType.HasValue
             ? new KdbndpParameter { DbType = ElementTypeMapping.DbType.Value }
                   .KdbndpDbType
             : default(KdbndpDbType?));
  }

  // This constructor exists only to support the static Default property above,
  // which is necessary to allow code generation for compiled models. The
  // constructor creates a completely blank type mapping, which will get cloned
  // with all the correct details.
  private KdbndpArrayTypeMapping()
      : base(new RelationalTypeMappingParameters(
            new CoreTypeMappingParameters(typeof(TCollection),
                                          elementMapping: NullMapping),
            "int[]")) {}
  ...
  public override DbParameter CreateParameter(
      DbCommand command, string name, object? value, bool? nullable = null,
      ParameterDirection direction = ParameterDirection.Input) {
    // In queries which compose non-server-correlated LINQ operators over an
    // array parameter (e.g. Where(b => ids.Skip(1)...) we get an enumerable
    // parameter value that isn't an array/list - but those aren't supported at
    // the Kdbndp ADO level. Detect this here and evaluate the enumerable to get
  ...
  protected override
    ...
    Check.DebugAssert(
        parameters.CoreParameters.ClrType == typeof(TCollection),
        "KdbndpArrayTypeMapping.Clone attempting to change ClrType");
    Check.DebugAssert(
        parameters.CoreParameters.ElementTypeMapping is not null,
        "KdbndpArrayTypeMapping.Clone without an element type mapping");
    Check.DebugAssert(
        parameters.CoreParameters.ElementTypeMapping.ClrType ==
            typeof(TElement).UnwrapNullableType(),
        "KdbndpArrayTypeMapping.Clone attempting to change element ClrType");

    return new KdbndpArrayTypeMapping<TCollection, TConcreteCollection,
                                      TElement>(parameters);
  }
  ...
  protected override string GenerateNonNullSqlLiteral(object value) {
    if (value is not IEnumerable enumerable) {
      throw new ArgumentException(
          $"'{value.GetType().Name}' must be an IEnumerable", nameof(value));
    }

    if (value is Array array && array.Rank != 1) {
      throw new NotSupportedException(
          "Multidimensional array literals aren't supported");
    }
