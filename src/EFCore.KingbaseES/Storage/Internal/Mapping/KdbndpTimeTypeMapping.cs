...
public class KdbndpTimeTypeMapping : KdbndpTypeMapping {
  ...
  public KdbndpTimeTypeMapping(Type clrType)
      : base("time without time zone", clrType, KdbndpDbType.Time,
             clrType == typeof(TimeOnly) ? JsonTimeOnlyReaderWriter.Instance
             : clrType == typeof(TimeSpan)
                 ? JsonTimeSpanReaderWriter.Instance
                 : throw new ArgumentException(
                       "clrType must be TimeOnly or TimeSpan",
                       nameof(clrType))) {}
  ...
  protected override string ProcessStoreType(
      ...
      string _) => parameters.Precision is null
                       ? storeType
                       : $"time({parameters.Precision}) without time zone";
  ...
  GenerateEmbeddedNonNullSqlLiteral(object value) => value switch {
    TimeSpan ts => ts.Ticks % 10000000 == 0
                       ? ts.ToString(@"hh\:mm\:ss",
                                     CultureInfo.InvariantCulture)
                       : ts.ToString(@"hh\:mm\:ss\.FFFFFF",
                                     CultureInfo.InvariantCulture),
    TimeOnly t => t.Ticks % 10000000 == 0
                      ? t.ToString(@"HH\:mm\:ss", CultureInfo.InvariantCulture)
                      : t.ToString(@"HH\:mm\:ss\.FFFFFF",
                                   CultureInfo.InvariantCulture),
    _ => throw new InvalidCastException(
        $"Can't generate a time SQL literal for CLR type {value.GetType()}")
  };
}
