...
public class KdbndpBigIntegerTypeMapping : KdbndpTypeMapping {
  ...
  public KdbndpBigIntegerTypeMapping()
      : base("numeric", typeof(BigInteger), KdbndpDbType.Numeric,
             jsonValueReaderWriter: JsonBigIntegerReaderWriter.Instance) {}
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  protected KdbndpBigIntegerTypeMapping(
  ...
  protected override string ProcessStoreType(
      ...
      string _) => parameters.Precision is null ? storeType
                   : parameters.Scale is null
                       ? $"numeric({parameters.Precision})"
                       : $"numeric({parameters.Precision},{parameters.Scale})";

  private sealed class JsonBigIntegerReaderWriter
      : JsonValueReaderWriter<BigInteger> {
    public static JsonBigIntegerReaderWriter Instance { get; } = new();

    // Other systems handling the JSON very likely won't support
    // arbitrary-length numbers here, we encode as a string
    public override BigInteger FromJsonTyped(ref Utf8JsonReaderManager manager,
