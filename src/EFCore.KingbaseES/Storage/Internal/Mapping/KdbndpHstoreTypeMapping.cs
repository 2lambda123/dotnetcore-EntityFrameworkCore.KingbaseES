...
/// <summary>
///     The type mapping for the KingbaseES hstore type. Supports both <see
///     cref="Dictionary{TKey,TValue} " /> and <see
///     cref="ImmutableDictionary{TKey,TValue}" /> over strings.
/// </summary>
/// <remarks>
...
public class KdbndpHstoreTypeMapping : KdbndpTypeMapping {
  ...
  private static ValueComparer? GetComparer(Type clrType) {
    ...
    if (clrType == typeof(ImmutableDictionary<string, string>)) {
      // Because ImmutableDictionary is immutable, we can use the default value
      // comparer, which doesn't clone for snapshot and just does reference
      // comparison. We could compare contents here if the references are
      // different, but that would penalize the 99% case where a different
      // reference means different contents, which would only save a very rare
      // database update.
      return null;
    }
  ...
  private sealed class HstoreMutableComparer
      : ValueComparer<Dictionary<string, string>> {
    public HstoreMutableComparer()
        : base((a, b) => Compare(a, b), o => o.GetHashCode(),
               o => new Dictionary<string, string>(o)) {}

    private static bool Compare(Dictionary<string, string>? a,
