...
public class KdbndpIntervalTypeMapping : KdbndpTypeMapping {
  ...
  public KdbndpIntervalTypeMapping()
      : this(new RelationalTypeMappingParameters(
            new CoreTypeMappingParameters(
                typeof(TimeSpan),
                jsonValueReaderWriter: KdbndpJsonTimeSpanReaderWriter.Instance),
            "interval")) {}
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  protected KdbndpIntervalTypeMapping(
  ...
  ProcessStoreType(RelationalTypeMappingParameters parameters, string storeType,
                   string _) => parameters.Precision is null
                                    ? storeType
                                    : $"interval({parameters.Precision})";
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  public static string FormatTimeSpanAsInterval(TimeSpan ts) => ts.ToString(
  ...
  public static TimeSpan ParseIntervalAsTimeSpan(ReadOnlySpan<char> s) {
    // We store the a PG-compatible interval representation in JSON so it can be
    // queried out, but unfortunately this isn't compatible with TimeSpan, so we
    ...
    // Space is the separator between the days and the hours:minutes:seconds
    // components
    var days = 0;
    if (s[i] == ' ') {
      days = int.Parse(s[..i]);
    ...
    // We've already extracted the days, so there shouldn't be a days component
    // in the rest (indicates an incompatible/malformed string)
    if (timeSpan.Days != 0) {
      throw new FormatException();
    }
    ...
    if (days > 0) {
      timeSpan =
          new TimeSpan(days, timeSpan.Hours, timeSpan.Minutes, timeSpan.Seconds,
                       timeSpan.Milliseconds, timeSpan.Microseconds);
    }
  ...
  private sealed class KdbndpJsonTimeSpanReaderWriter
      : JsonValueReaderWriter<TimeSpan> {
