...
public class KdbndpTimestampTzTypeMapping : KdbndpTypeMapping {
      ...
      : base("timestamp with time zone", clrType, KdbndpDbType.TimestampTz,
             ...
             : clrType == typeof(DateTimeOffset)
                 ...
                 : throw new ArgumentException(
                       "clrType must be DateTime or DateTimeOffset",
                       nameof(clrType))) {}
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  protected KdbndpTimestampTzTypeMapping(
  ...
  protected override string ProcessStoreType(
      RelationalTypeMappingParameters parameters, string storeType,
      string _) => parameters.Precision is null
                       ? storeType
                       : $"timestamp({parameters.Precision}) with time zone";
  ...
  private static string Format(DateTime dateTime) {
    ...
    return dateTime.Kind switch {
      DateTimeKind.Utc => dateTime.ToString("yyyy-MM-ddTHH:mm:ss.FFFFFF",
                                            CultureInfo.InvariantCulture) +
                          'Z',

      DateTimeKind.Unspecified =>
          KdbndpTypeMappingSource.LegacyTimestampBehavior || dateTime == default
              ? dateTime.ToString("yyyy-MM-ddTHH:mm:ss.FFFFFF",
                                  CultureInfo.InvariantCulture) +
                    'Z'
              : throw new ArgumentException(
      ...
      DateTimeKind.Local =>
          KdbndpTypeMappingSource.LegacyTimestampBehavior
              ? dateTime.ToString("yyyy-MM-ddTHH:mm:ss.FFFFFFzzz",
                                  CultureInfo.InvariantCulture)
              : throw new ArgumentException(
                    $"'timestamp with time zone' literal cannot be generated for {dateTime.Kind} DateTime: a UTC DateTime is required"),
  ...
  private sealed class KdbndpJsonTimestampTzDateTimeReaderWriter
      : JsonValueReaderWriter<DateTime> {
    ...
    public override DateTime FromJsonTyped(ref Utf8JsonReaderManager manager,
      ...
      // Our JSON string representation ends with Z (UTC), but DateTime.Parse
      // returns a Local timestamp even in that case. Convert it in order to
      // return a DateTime with Kind UTC.
  ...
  private sealed class KdbndpJsonTimestampTzDateTimeOffsetReaderWriter
      : JsonValueReaderWriter<DateTimeOffset> {
    public static KdbndpJsonTimestampTzDateTimeOffsetReaderWriter Instance {
      get;
    } = new();

    public override
        DateTimeOffset FromJsonTyped(ref Utf8JsonReaderManager manager,
                                     object? existingObject = null) {
      var s = manager.CurrentReader.GetString()!;
    ...
    ToJsonTyped(Utf8JsonWriter writer,
                DateTimeOffset value) => writer.WriteStringValue(Format(value));
  }
}
