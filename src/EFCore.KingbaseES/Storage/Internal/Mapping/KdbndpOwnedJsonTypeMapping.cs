...
/// <summary>
///     Supports the standard EF JSON support, which relies on owned entity
///     modeling. See <see cref="KdbndpJsonTypeMapping" /> for the older
///     Kdbndp-specific support, which allows mapping json/jsonb to text, to
///     e.g. <see cref="JsonElement" /> (weakly-typed mapping) or to arbitrary
///     POCOs (but without them being modeled).
/// </summary>
public class KdbndpOwnedJsonTypeMapping : JsonTypeMapping {
  /// <summary>
  ///     The database type used by Kdbndp (<see cref="KdbndpDbType.Json" /> or
  ///     <see cref="KdbndpDbType.Jsonb" />.
  /// </summary>
  public virtual KdbndpDbType KdbndpDbType { get; }

  private static readonly MethodInfo GetStringMethod =
      typeof(DbDataReader)
          .GetRuntimeMethod(nameof(DbDataReader.GetString),
                            new[] { typeof(int) })!;
  ...
  public KdbndpOwnedJsonTypeMapping(string storeType)
    ...
    KdbndpDbType = storeType switch {
      "json" => KdbndpDbType.Json, "jsonb" => KdbndpDbType.Jsonb,
      _ => throw new ArgumentException(
          "Only the json and jsonb types are supported", nameof(storeType))
    };
  }
  ...
  CustomizeDataReaderExpression(Expression expression) =>
      Expression.New(MemoryStreamConstructor,
                     Expression.Call(Expression.Property(null, UTF8Property),
                                     EncodingGetBytesMethod, expression));
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  protected KdbndpOwnedJsonTypeMapping(
      RelationalTypeMappingParameters parameters, KdbndpDbType kdbndpDbType)
      : base(parameters) {
  ...
  EscapeSqlLiteral(string literal) => literal.Replace("'", "''");
