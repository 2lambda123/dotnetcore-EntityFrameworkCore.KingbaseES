...
/// </summary>
public class KdbndpRelationalConnection : RelationalConnection,
                                          IKdbndpRelationalConnection {
  private readonly
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  protected KdbndpRelationalConnection(
      RelationalConnectionDependencies dependencies, DbDataSource? dataSource)
      : base(dependencies) {
    if (dataSource is not null) {
      _dataSource = dataSource;

#if DEBUG
      // We validate in KdbndpOptionsExtensions.Validate that DataSource and
      // these callbacks aren't specified together
      if (dependencies.ContextOptions.FindExtension<KdbndpOptionsExtension>()
              is {} KdbndpOptions) {
        Check.DebugAssert(
...
#endif
    } else if (dependencies.ContextOptions
                   .FindExtension<KdbndpOptionsExtension>()
                       is {} KdbndpOptions) {
      _provideClientCertificatesCallback =
          KdbndpOptions.ProvideClientCertificatesCallback;
      _remoteCertificateValidationCallback =
          KdbndpOptions.RemoteCertificateValidationCallback;
    }
  }
  ...
  protected override DbConnection CreateDbConnection() {
    ...
    if (_provideClientCertificatesCallback is not null) {
      conn.ProvideClientCertificatesCallback =
          _provideClientCertificatesCallback;
    }

    if (_remoteCertificateValidationCallback is not null) {
      conn.UserCertificateValidationCallback =
          _remoteCertificateValidationCallback;
    }
  ...
  ///     when updating to a new Entity Framework Core release.
  /// </summary>
  // TODO: Remove after DbDataSource support is added to EF Core
  // (https://github.com/dotnet/efcore/issues/28266)
  public override string? ConnectionString {
    get => _dataSource is null ? base.ConnectionString
...
public virtual IKdbndpRelationalConnection CreateAdminConnection() {
  if (Dependencies.ContextOptions.FindExtension<KdbndpOptionsExtension>()
          is not {} KdbndpOptions) {
    throw new InvalidOperationException(
        $"{nameof(KdbndpOptionsExtension)} not found in {nameof(CreateAdminConnection)}");
  }

  var adminConnectionString =
      new KdbndpConnectionStringBuilder(ConnectionString) {
        Database = KdbndpOptions.AdminDatabase ?? "postgres", Pooling = false,
        Multiplexing = false
      }
          .ToString();

  var adminKdbndpOptions =
      _dataSource is not null ? KdbndpOptions.WithConnection(
                                    ((KdbndpConnection)CreateDbConnection())
                                        .CloneWith(adminConnectionString))
      : KdbndpOptions.Connection is not null
          ? KdbndpOptions.WithConnection(
                DbConnection.CloneWith(adminConnectionString))
          : KdbndpOptions.WithConnectionString(adminConnectionString);

  var optionsBuilder = new DbContextOptionsBuilder();
  ((IDbContextOptionsBuilderInfrastructure)optionsBuilder)
      .AddOrUpdateExtension(adminKdbndpOptions);

  return new KdbndpRelationalConnection(
...
///     to a new Entity Framework Core release.
/// </summary>
// Accessing Transaction.Current is expensive, so don't do it if Enlist is false
// in the connection string
public override Transaction? CurrentAmbientTransaction =>
    ConnectionString is null ||
            !ConnectionString.Contains(
                "Enlist=false", StringComparison.InvariantCultureIgnoreCase)
        ? Transaction.Current
        : null;
...
public virtual KdbndpRelationalConnection CloneWith(string connectionString) {
  ...
  var relationalOptions =
      RelationalOptionsExtension.Extract(Dependencies.ContextOptions)
