...
public class KdbndpExecutionStrategy : IExecutionStrategy {
  ...
  public virtual TResult Execute<TState, TResult>(
    ...
    } catch (Exception ex) when (ExecutionStrategy.CallOnWrappedException(
        ex, KdbndpTransientExceptionDetector.ShouldRetryOn)) {
  ...
  public virtual async Task<TResult> ExecuteAsync<TState, TResult>(
      ...
      Func<DbContext, TState, CancellationToken,
           Task<ExecutionResult<TResult>>>? verifySucceeded,
      CancellationToken cancellationToken) {
    try {
      return await operation(Dependencies.CurrentContext.Context, state,
                             cancellationToken)
