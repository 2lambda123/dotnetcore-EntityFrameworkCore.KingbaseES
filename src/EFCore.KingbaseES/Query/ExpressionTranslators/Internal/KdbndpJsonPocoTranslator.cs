...
public class KdbndpJsonPocoTranslator : IMemberTranslator,
  ...
  public virtual SqlExpression? Translate(
    ...
    if (method.IsClosedFormOf(Enumerable_AnyWithoutPredicate) &&
        TranslateArrayLength(arguments[0])
            is SqlExpression arrayLengthTranslation) {
      return _sqlExpressionFactory.GreaterThan(
          arrayLengthTranslation, _sqlExpressionFactory.Constant(0));
    }
  ...
  public virtual SqlExpression? Translate(
      ...
      IDiagnosticsLogger<DbLoggerCategory.Query> logger) {
    if (instance?.TypeMapping is not KdbndpJsonTypeMapping &&
        instance is not PgJsonTraversalExpression) {
      return null;
    }
  ...
  public virtual SqlExpression? TranslateMemberAccess(
      SqlExpression instance, SqlExpression member,
      Type returnType) => instance switch {
    // The first time we see a JSON traversal it's on a column - create a
    // JsonTraversalExpression. Traversals on top of that get appended into the
    // same expression.
    ColumnExpression { TypeMapping : KdbndpJsonTypeMapping } columnExpression =>
        ConvertFromText(_sqlExpressionFactory.JsonTraversal(columnExpression,
                                                            new[] { member },
                                                            returnsText: true,
                                                            typeof(string),
                                                            _stringTypeMapping),
    ...
    PgJsonTraversalExpression prevPathTraversal => ConvertFromText(
        prevPathTraversal.Append(
            _sqlExpressionFactory.ApplyDefaultTypeMapping(member)),
        returnType),
  ...
  public virtual SqlExpression? TranslateArrayLength(SqlExpression expression) {
    switch (expression) {
    case ColumnExpression { TypeMapping:
      KdbndpJsonTypeMapping mapping
    }:
  ...
  case PgJsonTraversalExpression traversal:
    // The traversal expression has ReturnsText=true (e.g. ->> not ->), so we
    // recreate it to return the JSON object instead.
    var lastPathComponent = traversal.Path.Last();
    var newTraversal = new PgJsonTraversalExpression(
        traversal.Expression, traversal.Path, returnsText: false,
    ...
    return _sqlExpressionFactory.Function(
        ...
        new[] { newTraversal }, nullable: true,
        argumentsPropagateNullability: TrueArrays[2], typeof(int));
...
// The KingbaseES traversal operator always returns text, so we need to convert
// to int, bool, etc.
