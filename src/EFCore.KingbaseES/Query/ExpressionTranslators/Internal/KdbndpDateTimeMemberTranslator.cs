        ...
        IDiagnosticsLogger<DbLoggerCategory.Query> logger)
    {
        ...
        if (member.Name == nameof(DateTime.Date))
        {
            ...
            case {
                ...
                when KdbndpTypeMappingSource.LegacyTimestampBehavior:
    ...
    => member.Name switch
{
    ...
    private bool TryConvertAwayFromTimestampTz(SqlExpression timestamp, [NotNullWhen(true)] out SqlExpression? result)
    {
        ...
        case {
                TypeMapping:
                KdbndpTimestampTypeMapping or KdbndpDateTimeDateTypeMapping or KdbndpTimeTypeMapping
            }:
        case {
                Type:
                var type
            }
            when type == typeof(DateOnly) || type == typeof(TimeOnly):
                result = timestamp;
            return true;
