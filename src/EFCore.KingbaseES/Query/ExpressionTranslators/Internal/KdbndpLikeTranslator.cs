...
/// <summary>
///     Translates <see cref="T:DbFunctionsExtensions.Like" /> methods into
///     KingbaseES LIKE expressions.
/// </summary>
public class KdbndpLikeTranslator : IMethodCallTranslator {
  ...
  private static readonly MethodInfo LikeWithEscape =
      typeof(DbFunctionsExtensions)
          .GetRuntimeMethod(nameof(DbFunctionsExtensions.Like),
  ...
  private static readonly MethodInfo ILikeWithEscape =
      typeof(KdbndpDbFunctionsExtensions)
          .GetRuntimeMethod(nameof(KdbndpDbFunctionsExtensions.ILike),
                            new[] { typeof(DbFunctions), typeof(string),
                                    typeof(string), typeof(string) })!;
  ...
  /// <summary>
  ///     Initializes a new instance of the <see cref="KdbndpMathTranslator" />
  ///     class.
  /// </summary>
  /// <param name="sqlExpressionFactory">The SQL expression factory to use when
  /// generating expressions..</param>
  ...
  public virtual SqlExpression? Translate(
    ...
    // KingbaseES has backslash as the default LIKE escape character, but EF
    // Core expects no escape character unless explicitly requested
    // (https://github.com/aspnet/EntityFramework/issues/8696).

    // If we have a constant expression, we check that there are no backslashes
    // in order to render with an ESCAPE clause (better SQL). If we have a
    // constant expression with backslashes or a non-constant expression, we
    // render an ESCAPE clause to disable backslash escaping.
    ...
    if (pattern is SqlConstantExpression { Value : string patternValue } &&
      ...
      return sensitive ? _sqlExpressionFactory.Like(match, pattern)
                       : _sqlExpressionFactory.ILike(match, pattern);
    }

    return sensitive ? _sqlExpressionFactory.Like(
                           match, pattern,
