...
/// <remarks>
///     See:
///     https://www.KingbaseES.org/docs/current/static/functions-string.html
/// </remarks>
...
#region MethodInfo
  ...
  private static readonly MethodInfo IndexOfChar =
      typeof(string).GetRuntimeMethod(nameof(string.IndexOf),
                                      new[] { typeof(char) })!;
  ...
  private static readonly MethodInfo IsNullOrWhiteSpace =
      typeof(string).GetRuntimeMethod(nameof(string.IsNullOrWhiteSpace),
                                      new[] { typeof(string) })!;

  private static readonly MethodInfo PadLeft = typeof(string).GetRuntimeMethod(
      nameof(string.PadLeft), new[] { typeof(int) })!;

  private static readonly MethodInfo PadLeftWithChar =
      typeof(string).GetRuntimeMethod(nameof(string.PadLeft),
                                      new[] { typeof(int), typeof(char) })!;

  private static readonly MethodInfo PadRight = typeof(string).GetRuntimeMethod(
      nameof(string.PadRight), new[] { typeof(int) })!;

  private static readonly MethodInfo PadRightWithChar =
      typeof(string).GetRuntimeMethod(nameof(string.PadRight),
                                      new[] { typeof(int), typeof(char) })!;
  ...
  private static readonly MethodInfo ToLower = typeof(string).GetRuntimeMethod(
      nameof(string.ToLower), Array.Empty<Type>())!;
  private static readonly MethodInfo ToUpper = typeof(string).GetRuntimeMethod(
      nameof(string.ToUpper), Array.Empty<Type>())!;
  ...
  private static readonly MethodInfo Reverse =
      typeof(KdbndpDbFunctionsExtensions)
          .GetRuntimeMethod(nameof(KdbndpDbFunctionsExtensions.Reverse),
                            new[] { typeof(DbFunctions), typeof(string) })!;

  private static readonly MethodInfo StringToArray =
      typeof(KdbndpDbFunctionsExtensions)
          .GetRuntimeMethod(
              nameof(KdbndpDbFunctionsExtensions.StringToArray),
              new[] { typeof(DbFunctions), typeof(string), typeof(string) })!;

  private static readonly MethodInfo StringToArrayNullString =
      typeof(KdbndpDbFunctionsExtensions)
          .GetRuntimeMethod(nameof(KdbndpDbFunctionsExtensions.StringToArray),
                            new[] { typeof(DbFunctions), typeof(string),
  ...
  private static readonly MethodInfo LastOrDefaultMethodInfoWithoutArgs =
      typeof(Enumerable)
          .GetRuntimeMethods()
          .Single(m => m.Name == nameof(Enumerable.LastOrDefault) &&
                       m.GetParameters().Length == 1)
          .MakeGenericMethod(typeof(char));

  // ReSharper disable InconsistentNaming
  private static readonly MethodInfo String_Join1 = typeof(string).GetMethod(
      nameof(string.Join), new[] { typeof(string), typeof(object[]) })!;

  private static readonly MethodInfo String_Join2 = typeof(string).GetMethod(
      nameof(string.Join), new[] { typeof(string), typeof(string[]) })!;

  private static readonly MethodInfo String_Join3 = typeof(string).GetMethod(
      nameof(string.Join), new[] { typeof(char), typeof(object[]) })!;

  private static readonly MethodInfo String_Join4 = typeof(string).GetMethod(
      nameof(string.Join), new[] { typeof(char), typeof(string[]) })!;
  ...
  private static readonly MethodInfo String_Join_generic1 =
      typeof(string)
          ...
          .Single(
              m => m
                   is { Name : nameof(string.Join), IsGenericMethod : true } &&
  ...
  public virtual SqlExpression? Translate(
    ...
    if (method == FirstOrDefaultMethodInfoWithoutArgs) {
      ...
      return _sqlExpressionFactory.Function(
          "substr",
          new[] { argument, _sqlExpressionFactory.Constant(1),
                  _sqlExpressionFactory.Constant(1) },
          nullable: true, argumentsPropagateNullability: TrueArrays[3],
          method.ReturnType);
    }
      ...
      return _sqlExpressionFactory.Function(
          ...
          new[] { argument,
                  _sqlExpressionFactory.Function(
                      "length", new[] { argument }, nullable: true,
  ...
  private SqlExpression? TranslateStringMethod(
    ...
    if (method == IndexOfString || method == IndexOfChar) {
      ...
      var stringTypeMapping =
          ExpressionExtensions.InferTypeMapping(instance!, argument);

      return _sqlExpressionFactory.Subtract(
          _sqlExpressionFactory.Function(
              "strpos",
              new[] { _sqlExpressionFactory.ApplyTypeMapping(instance!,
                                                             stringTypeMapping),
                      _sqlExpressionFactory.ApplyTypeMapping(
                          argument, stringTypeMapping) },
    ...
    if (method == Replace) {
      ...
      var stringTypeMapping =
          ExpressionExtensions.InferTypeMapping(instance!, oldValue, newValue);

      return _sqlExpressionFactory.Function(
          "replace",
          new[] {
            _sqlExpressionFactory.ApplyTypeMapping(instance!,
                                                   stringTypeMapping),
          ...
          nullable: true, argumentsPropagateNullability: TrueArrays[3],
          method.ReturnType, stringTypeMapping);
    }

    if (method == ToLower || method == ToUpper) {
      return _sqlExpressionFactory.Function(
          method == ToLower ? "lower" : "upper", new[] { instance! },
    ...
    if (method == IsNullOrWhiteSpace) {
      ...
      return _sqlExpressionFactory.OrElse(
          ...
          _sqlExpressionFactory.Equal(
              _sqlExpressionFactory.Function(
                  "btrim", new[] { argument, _whitespace }, nullable: true,
                  argumentsPropagateNullability: TrueArrays[2], argument.Type,
                  argument.TypeMapping),
              _sqlExpressionFactory.Constant(string.Empty,
                                             argument.TypeMapping)));
    }

    var isTrimStart = method == TrimStartWithNoParam ||
                      method == TrimStartWithChars ||
                      method == TrimStartWithSingleChar;
    var isTrimEnd = method == TrimEndWithNoParam ||
                    method == TrimEndWithChars ||
                    method == TrimEndWithSingleChar;
    var isTrimBoth = method == TrimBothWithNoParam ||
    ...
    if (method == PadLeft || method == PadLeftWithChar || method == PadRight ||
        method == PadRightWithChar) {
      var args = method == PadLeft || method == PadRight
    ...
    if (method.DeclaringType == typeof(string) &&
        (method == String_Join1 || method == String_Join2 ||
         method == String_Join3 || method == String_Join4 ||
         method.IsClosedFormOf(String_Join_generic1) ||
         method.IsClosedFormOf(String_Join_generic2))) {
      // If the array of strings to be joined is a constant
      // (NewArrayExpression), we translate to concat_ws. Otherwise we translate
      // to array_to_string, which also supports array columns and parameters.
      if (arguments[1] is PgNewArrayExpression newArrayExpression) {
        var rewrittenArguments =
            new SqlExpression[newArrayExpression.Expressions.Count + 1];
        ...
        for (var i = 0; i < newArrayExpression.Expressions.Count; i++) {
          ...
          rewrittenArguments[i + 1] = argument switch {
            ...
            SqlConstantExpression constantExpression =>
                constantExpression.Value is null
                    ? _sqlExpressionFactory.Constant(string.Empty,
                                                     typeof(string))
                    : constantExpression,
            _ => _sqlExpressionFactory.Coalesce(
                argument,
                _sqlExpressionFactory.Constant(string.Empty, typeof(string)))
          };
        }

        // Only the delimiter (first arg) propagates nullability - all others
        // are non-nullable, since we wrap the others in coalesce (where
        // needed).
        var argumentsPropagateNullability = new bool[rewrittenArguments.Length];
        argumentsPropagateNullability[0] = true;

        return _sqlExpressionFactory.Function(
            "concat_ws", rewrittenArguments, nullable: true,
  ...
  private SqlExpression? TranslateDbFunctionsMethod(
      SqlExpression? instance, MethodInfo method,
      IReadOnlyList<SqlExpression> arguments) {
    if (method == Reverse) {
      return _sqlExpressionFactory.Function(
          "reverse", new[] { arguments[1] }, nullable: true,
          argumentsPropagateNullability: TrueArrays[1], typeof(string),
          arguments[1].TypeMapping);
    }

    if (method == StringToArray) {
      // Note that string_to_array always returns text[], regardless of the
      // input type
      return _sqlExpressionFactory.Function(
          "string_to_array", new[] { arguments[1], arguments[2] },
          nullable: true, argumentsPropagateNullability: new[] { true, false },
    ...
    if (method == StringToArrayNullString) {
      ...
      // input type
      return _sqlExpressionFactory.Function(
          "string_to_array", new[] { arguments[1], arguments[2], arguments[3] },
          nullable: true,
