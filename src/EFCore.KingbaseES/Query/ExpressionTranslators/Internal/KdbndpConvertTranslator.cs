...
/// <summary>
///     Translates methods defined on <see cref="T:System.Convert" /> into
///     KingbaseES CAST expressions.
/// </summary>
public class KdbndpConvertTranslator : IMethodCallTranslator {
  private static readonly Dictionary<string, string> TypeMapping =
      new() { [nameof(Convert.ToBoolean)] = "bool",
  ...
  private static readonly List<Type> SupportedTypes =
      new() { typeof(bool),   typeof(byte),  typeof(decimal),
              typeof(double), typeof(float), typeof(int),
              typeof(long),   typeof(short), typeof(string) };

  private static readonly List<MethodInfo> SupportedMethods =
      TypeMapping.Keys
          .SelectMany(
              t => typeof(Convert).GetTypeInfo().GetDeclaredMethods(t).Where(
  ...
  public virtual SqlExpression? Translate(
      ...
      IDiagnosticsLogger<DbLoggerCategory.Query>
          logger) => SupportedMethods.Contains(method)
                         ? _sqlExpressionFactory.Convert(arguments[0],
                                                         method.ReturnType)
                         : null;
}
