...
public class KdbndpRowValueTranslator : IMethodCallTranslator {
  ...
  private static readonly MethodInfo GreaterThanOrEqual =
      typeof(KdbndpDbFunctionsExtensions)
          .GetMethods()
          .Single(m => m.Name ==
                       nameof(KdbndpDbFunctionsExtensions.GreaterThanOrEqual));

  private static readonly MethodInfo LessThanOrEqual =
      typeof(KdbndpDbFunctionsExtensions)
          .GetMethods()
          .Single(m => m.Name ==
                       nameof(KdbndpDbFunctionsExtensions.LessThanOrEqual));

  private static readonly Dictionary<MethodInfo, ExpressionType>
      ComparisonMethods =
          new() { { GreaterThan, ExpressionType.GreaterThan },
                  { LessThan, ExpressionType.LessThan },
                  { GreaterThanOrEqual, ExpressionType.GreaterThanOrEqual },
                  { LessThanOrEqual, ExpressionType.LessThanOrEqual } };

  /// <summary>
  ///     Initializes a new instance of the <see cref="KdbndpRowValueTranslator"
  ///     /> class.
  /// </summary>
  public KdbndpRowValueTranslator(
  ...
  [DynamicDependency(DynamicallyAccessedMemberTypes.PublicMethods,
                     typeof(ValueType))] // For ValueTuple.Create
  public virtual SqlExpression? Translate(
      SqlExpression? instance, MethodInfo method,
      IReadOnlyList<SqlExpression> arguments,
      IDiagnosticsLogger<DbLoggerCategory.Query> logger) {
    // Translate ValueTuple.Create
    if (method.DeclaringType == typeof(ValueTuple) && method
        is { IsStatic : true, Name : nameof(ValueTuple.Create) }) {
    ...
    if (method.DeclaringType != typeof(KdbndpDbFunctionsExtensions) ||
        !ComparisonMethods.TryGetValue(method, out var expressionType)) {
      return null;
    }
    ...
    var leftCount = arguments[1] is PgRowValueExpression leftRowValue
                    ...
                    : arguments[1] is
                        SqlConstantExpression { Value : ITuple leftTuple }
    ...
    var rightCount = arguments[2] is PgRowValueExpression rightRowValue
                     ...
                     : arguments[2] is
                         SqlConstantExpression { Value : ITuple rightTuple }
    ...
    return _sqlExpressionFactory.MakeBinary(expressionType, arguments[1],
                                            arguments[2], typeMapping: null);
  }
}
