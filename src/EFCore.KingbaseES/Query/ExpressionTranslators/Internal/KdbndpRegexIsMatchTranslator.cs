...
/// <summary>
///     Translates Regex.IsMatch calls into KingbaseES regex expressions for
...
public class KdbndpRegexIsMatchTranslator : IMethodCallTranslator {
  private static readonly MethodInfo IsMatch = typeof(Regex).GetRuntimeMethod(
      nameof(Regex.IsMatch), new[] { typeof(string), typeof(string) })!;

  private static readonly MethodInfo IsMatchWithRegexOptions =
      typeof(Regex).GetRuntimeMethod(
          nameof(Regex.IsMatch),
          new[] { typeof(string), typeof(string), typeof(RegexOptions) })!;

  private const RegexOptions UnsupportedRegexOptions =
      RegexOptions.RightToLeft | RegexOptions.ECMAScript;
  ...
  /// </summary>
  public KdbndpRegexIsMatchTranslator(
  ...
  public virtual SqlExpression? Translate(
    ...
    } else if (arguments[2] is
               SqlConstantExpression { Value : RegexOptions regexOptions }) {
    ...
    return (options & UnsupportedRegexOptions) == 0
               ? _sqlExpressionFactory.RegexMatch(
                     ...
                     _sqlExpressionFactory.ApplyTypeMapping(pattern,
                                                            typeMapping),
                     options)
               : null;
  }
