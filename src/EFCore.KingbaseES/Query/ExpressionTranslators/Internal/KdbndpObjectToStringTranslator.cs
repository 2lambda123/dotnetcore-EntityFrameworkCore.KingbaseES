...
public class KdbndpObjectToStringTranslator : IMethodCallTranslator {
  ...
  /// </summary>
  public KdbndpObjectToStringTranslator(
  ...
  public virtual SqlExpression? Translate(
    ...
    if (instance is null || method.Name != nameof(ToString) ||
        arguments.Count != 0) {
      return null;
    }
    ...
    if (instance.Type == typeof(bool)) {
      return instance is ColumnExpression { IsNullable : true }
                 ...
                 : _sqlExpressionFactory.Case(
                       new[] { new CaseWhenClause(
                           _sqlExpressionFactory.Equal(
                               instance, _sqlExpressionFactory.Constant(false)),
                           _sqlExpressionFactory.Constant(false.ToString())) },
                       _sqlExpressionFactory.Constant(true.ToString()));
    }

    return _typeMapping.Contains(instance.Type) ||
                   instance.Type.UnwrapNullableType().IsEnum &&
                       instance.TypeMapping is KdbndpEnumTypeMapping
