...
/// </summary>
public class KdbndpStatisticsAggregateMethodTranslator
  ...
  public virtual SqlExpression? Translate(
    ...
    // Docs:
    // https://www.KingbaseES.org/docs/current/functions-aggregate.html#FUNCTIONS-AGGREGATE-STATISTICS-TABLE
    ...
    var functionName = method.Name switch {
      nameof(KdbndpAggregateDbFunctionsExtensions.StandardDeviationSample) =>
          "stddev_samp",
      nameof(
          KdbndpAggregateDbFunctionsExtensions.StandardDeviationPopulation) =>
          "stddev_pop",
      nameof(KdbndpAggregateDbFunctionsExtensions.VarianceSample) => "var_samp",
      nameof(KdbndpAggregateDbFunctionsExtensions.VariancePopulation) =>
          "var_pop",
      _ => null
    };
    ...
    if (functionName is not null) {
      return _sqlExpressionFactory.AggregateFunction(
          functionName, new[] { sqlExpression }, source, nullable: true,
          argumentsPropagateNullability: FalseArrays[1], typeof(double),
          _doubleTypeMapping);
    }

    functionName = method.Name switch {
      nameof(KdbndpAggregateDbFunctionsExtensions.Correlation) => "corr",
      nameof(KdbndpAggregateDbFunctionsExtensions.CovariancePopulation) =>
          "covar_pop",
      nameof(KdbndpAggregateDbFunctionsExtensions.CovarianceSample) =>
          "covar_samp",
      ...
      nameof(KdbndpAggregateDbFunctionsExtensions.RegrIntercept) =>
          "regr_intercept",
    ...
    if (functionName is not null) {
      ...
      // represented in LINQ as a projection from the grouping to a tuple of the
      // two columns. Since we generally translate tuples to
      ...
      return method.Name ==
                     nameof(KdbndpAggregateDbFunctionsExtensions.RegrCount)
                 ? _sqlExpressionFactory.AggregateFunction(
                       functionName, new[] { y, x }, source, nullable: true,
