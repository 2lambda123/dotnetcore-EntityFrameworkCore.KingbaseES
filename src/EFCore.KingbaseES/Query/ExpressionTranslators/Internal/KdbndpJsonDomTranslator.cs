...
/// </summary>
public class KdbndpJsonDomTranslator : IMemberTranslator,
                                       IMethodCallTranslator {
  private static readonly MemberInfo RootElement =
      typeof(JsonDocument).GetProperty(nameof(JsonDocument.RootElement))!;

  private static readonly MethodInfo GetProperty =
      typeof(JsonElement)
          .GetRuntimeMethod(nameof(JsonElement.GetProperty),
                            new[] { typeof(string) })!;

  private static readonly MethodInfo GetArrayLength =
      typeof(JsonElement)
          .GetRuntimeMethod(nameof(JsonElement.GetArrayLength),
                            Type.EmptyTypes)!;

  private static readonly MethodInfo ArrayIndexer =
      typeof(JsonElement)
          .GetProperties()
  ...
  private static readonly string[] GetMethods = {
    nameof(JsonElement.GetBoolean),        nameof(JsonElement.GetDateTime),
    nameof(JsonElement.GetDateTimeOffset), nameof(JsonElement.GetDecimal),
    nameof(JsonElement.GetDouble),         nameof(JsonElement.GetGuid),
    nameof(JsonElement.GetInt16),          nameof(JsonElement.GetInt32),
    nameof(JsonElement.GetInt64),          nameof(JsonElement.GetSingle),
  ...
  public virtual SqlExpression? Translate(
    ...
    if (member == RootElement && instance is
        ColumnExpression { TypeMapping : KdbndpJsonTypeMapping } column) {
  ...
  public virtual SqlExpression? Translate(
    ...
    // The root of the JSON expression is a ColumnExpression. We wrap that with
    // an empty traversal expression (col #>> '{}'); subsequent traversals will
    // gradually append the path into that. Note that it's possible to call
    // methods such as GetString() directly on the root, and the empty traversal
    // is necessary to properly convert it to a text.
    instance =
    ...
    if (GetMethods.Contains(method.Name) && arguments.Count == 0 &&
        instance is PgJsonTraversalExpression traversal) {
      var traversalToText = new PgJsonTraversalExpression(
          traversal.Expression, traversal.Path, returnsText: true,
    ...
    if (method == GetArrayLength) {
      return _sqlExpressionFactory.Function(
          ...
          new[] { instance }, nullable: true,
          argumentsPropagateNullability: TrueArrays[1], typeof(int));
    }

    if (method.Name.StartsWith("TryGet", StringComparison.Ordinal) &&
        arguments.Count == 0) {
  ...
  // The KingbaseES traversal operator always returns text, so we need to
  // convert to int, bool, etc.
  private SqlExpression ConvertFromText(SqlExpression expression,
                                        Type returnType) {
