...
public class KdbndpDateTimeMethodTranslator : IMethodCallTranslator {
  private static readonly Dictionary<MethodInfo, string>
      MethodInfoDatePartMapping = new() {
        { typeof(DateTime).GetRuntimeMethod(nameof(DateTime.AddYears),
                                            new[] { typeof(int) })!,
          "years" },
        { typeof(DateTime).GetRuntimeMethod(nameof(DateTime.AddMonths),
                                            new[] { typeof(int) })!,
          "months" },
        { typeof(DateTime).GetRuntimeMethod(nameof(DateTime.AddDays),
                                            new[] { typeof(double) })!,
          "days" },
        { typeof(DateTime).GetRuntimeMethod(nameof(DateTime.AddHours),
                                            new[] { typeof(double) })!,
          "hours" },
        { typeof(DateTime).GetRuntimeMethod(nameof(DateTime.AddMinutes),
                                            new[] { typeof(double) })!,
          "mins" },
        { typeof(DateTime).GetRuntimeMethod(nameof(DateTime.AddSeconds),
                                            new[] { typeof(double) })!,
          "secs" },
        //{ typeof(DateTime).GetRuntimeMethod(nameof(DateTime.AddMilliseconds),
        //new[] { typeof(double) })!, "milliseconds" },

        { typeof(DateTimeOffset)
              .GetRuntimeMethod(nameof(DateTimeOffset.AddYears),
                                new[] { typeof(int) })!,
          "years" },
        { typeof(DateTimeOffset)
              .GetRuntimeMethod(nameof(DateTimeOffset.AddMonths),
                                new[] { typeof(int) })!,
          "months" },
        { typeof(DateTimeOffset)
              .GetRuntimeMethod(nameof(DateTimeOffset.AddDays),
                                new[] { typeof(double) })!,
          "days" },
        { typeof(DateTimeOffset)
              .GetRuntimeMethod(nameof(DateTimeOffset.AddHours),
                                new[] { typeof(double) })!,
          "hours" },
        { typeof(DateTimeOffset)
              .GetRuntimeMethod(nameof(DateTimeOffset.AddMinutes),
                                new[] { typeof(double) })!,
          "mins" },
        { typeof(DateTimeOffset)
              .GetRuntimeMethod(nameof(DateTimeOffset.AddSeconds),
                                new[] { typeof(double) })!,
          "secs" },
        //{
        //typeof(DateTimeOffset).GetRuntimeMethod(nameof(DateTimeOffset.AddMilliseconds),
        //new[] { typeof(double) })!, "milliseconds" }

        // DateOnly.AddDays, AddMonths and AddYears have a specialized
        // translation, see below
        { typeof(TimeOnly).GetRuntimeMethod(nameof(TimeOnly.AddHours),
                                            new[] { typeof(int) })!,
          "hours" },
        { typeof(TimeOnly).GetRuntimeMethod(nameof(TimeOnly.AddMinutes),
                                            new[] { typeof(int) })!,
          "mins" },
      };
  ...
  private static readonly MethodInfo DateTime_SpecifyKind =
      typeof(DateTime).GetRuntimeMethod(
          nameof(DateTime.SpecifyKind),
          new[] { typeof(DateTime), typeof(DateTimeKind) })!;

  private static readonly MethodInfo DateTime_Distance =
      typeof(KdbndpDbFunctionsExtensions)
          .GetRuntimeMethod(nameof(KdbndpDbFunctionsExtensions.Distance),
                            new[] { typeof(DbFunctions), typeof(DateTime),
                                    typeof(DateTime) })!;

  private static readonly MethodInfo DateOnly_FromDateTime =
      typeof(DateOnly).GetRuntimeMethod(nameof(DateOnly.FromDateTime),
                                        new[] { typeof(DateTime) })!;

  private static readonly MethodInfo DateOnly_ToDateTime =
      typeof(DateOnly).GetRuntimeMethod(nameof(DateOnly.ToDateTime),
                                        new[] { typeof(TimeOnly) })!;

  private static readonly MethodInfo DateOnly_Distance =
      typeof(KdbndpDbFunctionsExtensions)
          .GetRuntimeMethod(nameof(KdbndpDbFunctionsExtensions.Distance),
                            new[] { typeof(DbFunctions), typeof(DateOnly),
                                    typeof(DateOnly) })!;

  private static readonly MethodInfo DateOnly_AddDays =
      typeof(DateOnly).GetRuntimeMethod(nameof(DateOnly.AddDays),
                                        new[] { typeof(int) })!;

  private static readonly MethodInfo DateOnly_AddMonths =
      typeof(DateOnly).GetRuntimeMethod(nameof(DateOnly.AddMonths),
                                        new[] { typeof(int) })!;

  private static readonly MethodInfo DateOnly_AddYears =
      typeof(DateOnly).GetRuntimeMethod(nameof(DateOnly.AddYears),
                                        new[] { typeof(int) })!;

  private static readonly MethodInfo TimeOnly_FromDateTime =
      typeof(TimeOnly).GetRuntimeMethod(nameof(TimeOnly.FromDateTime),
                                        new[] { typeof(DateTime) })!;

  private static readonly MethodInfo TimeOnly_FromTimeSpan =
      typeof(TimeOnly).GetRuntimeMethod(nameof(TimeOnly.FromTimeSpan),
                                        new[] { typeof(TimeSpan) })!;

  private static readonly MethodInfo TimeOnly_ToTimeSpan =
      typeof(TimeOnly).GetRuntimeMethod(nameof(TimeOnly.ToTimeSpan),
                                        Type.EmptyTypes)!;
  ...
  private static readonly MethodInfo TimeOnly_Add_TimeSpan =
      typeof(TimeOnly).GetRuntimeMethod(nameof(TimeOnly.Add),
                                        new[] { typeof(TimeSpan) })!;
  ...
  private static readonly MethodInfo TimeZoneInfo_ConvertTimeToUtc =
      typeof(TimeZoneInfo)
          .GetRuntimeMethod(nameof(TimeZoneInfo.ConvertTimeToUtc),
                            new[] { typeof(DateTime) })!;
  ...
  public KdbndpDateTimeMethodTranslator(
    ...
    _timestampMapping =
        typeMappingSource.FindMapping("timestamp without time zone")!;
    _timestampTzMapping =
        typeMappingSource.FindMapping("timestamp with time zone")!;
  ...
  public virtual SqlExpression? Translate(
      ...
      IDiagnosticsLogger<DbLoggerCategory.Query>
          logger) => TranslateDateTime(instance, method, arguments) ??
  ...
  private SqlExpression? TranslateDatePart(
      ...
      instance is not null &&
              ...
              CreateIntervalExpression(arguments[0],
                                       datePart) is SqlExpression interval
  ...
  private SqlExpression? TranslateDateTime(
    ...
    if (instance is null) {
      if (method == DateTime_SpecifyKind) {
        if (arguments[1] is not
            SqlConstantExpression { Value : DateTimeKind kind }) {
          throw new InvalidOperationException(
              "Translating SpecifyKind is only supported with a constant Kind argument");
        }

        var typeMapping = arguments[0].TypeMapping;

        if (typeMapping is not KdbndpTimestampTypeMapping and not
                KdbndpTimestampTzTypeMapping) {
          throw new InvalidOperationException(
              "Translating SpecifyKind is only supported on timestamp/timestamptz columns");
        }
      ...
      if (method == DateTime_Distance) {
        return _sqlExpressionFactory.MakePostgresBinary(
            PgExpressionType.Distance, arguments[1], arguments[2]);
      }
    } else {
  ...
  private SqlExpression? TranslateDateOnly(
    ...
    if (instance is null) {
      if (method == DateOnly_FromDateTime) {
        // Note: converting timestamptz to date performs a timezone conversion,
        // which is not what .NET DateOnly.FromDateTime does. So if our operand
        // is a timestamptz, we first change the type to timestamp with AT TIME
        // ZONE 'UTC' (returns the same value but as a timestamptz). If our
        // operand is already timestamp, no need to do anything. We throw for
        // anything else to avoid accidentally applying AT TIME ZONE to a
        // non-timestamptz, which would do a timezone conversion
        var dateTime = arguments[0].TypeMapping switch {
          KdbndpTimestampTypeMapping => arguments[0],
          KdbndpTimestampTzTypeMapping =>
              _sqlExpressionFactory.AtUtc(arguments[0]),
          _ => throw new NotSupportedException(
              "Can only apply TimeOnly.FromDateTime on a timestamp or timestamptz column")
        };

        return _sqlExpressionFactory.Convert(
            dateTime, typeof(DateOnly),
      ...
      // For months and years, date + interval yields a timestamp (since
      // interval could have a time component), so we need to cast the results
      // back to date
      if (method == DateOnly_AddMonths &&
          CreateIntervalExpression(arguments[0], "months")
              is SqlExpression interval1) {
        return _sqlExpressionFactory.Convert(
            _sqlExpressionFactory.Add(instance, interval1,
                                      instance.TypeMapping),
            typeof(DateOnly));
      }

      if (method == DateOnly_AddYears &&
          CreateIntervalExpression(arguments[0], "years")
              is SqlExpression interval2) {
        return _sqlExpressionFactory.Convert(
            _sqlExpressionFactory.Add(instance, interval2,
                                      instance.TypeMapping),
            typeof(DateOnly));
      }
    }
  ...
  private SqlExpression? TranslateTimeOnly(
    ...
    if (method == TimeOnly_FromDateTime) {
      // Note: converting timestamptz to time performs a timezone conversion,
      // which is not what .NET TimeOnly.FromDateTime does. So if our operand is
      // a timestamptz, we first change the type to timestamp with AT TIME ZONE
      // 'UTC' (returns the same value but as a timestamptz). If our operand is
      // already timestamp, no need to do anything. We throw for anything else
      // to avoid accidentally applying AT TIME ZONE to a non-timestamptz, which
      // would do a timezone conversion
      var dateTime = arguments[0].TypeMapping switch {
        KdbndpTimestampTypeMapping => arguments[0],
        KdbndpTimestampTzTypeMapping =>
            _sqlExpressionFactory.AtUtc(arguments[0]),
  ...
  private SqlExpression? TranslateTimeZoneInfo(
    ...
    if (method == TimeZoneInfo_ConvertTimeBySystemTimeZoneId_DateTime) {
      ...
      if (typeMapping is null ||
          (typeMapping.StoreType != "timestamp with time zone" &&
           typeMapping.StoreType != "timestamptz")) {
        throw new InvalidOperationException(
      ...
      return _sqlExpressionFactory.AtTimeZone(
          arguments[0], arguments[1], typeof(DateTime), _timestampMapping);
    }

    if (method == TimeZoneInfo_ConvertTimeToUtc) {
      var typeMapping = arguments[0].TypeMapping;
      if (typeMapping is null ||
          (typeMapping.StoreType != "timestamp without time zone" &&
           typeMapping.StoreType != "timestamp")) {
        throw new InvalidOperationException(
            "TimeZoneInfo.ConvertTimeToUtc) is only supported on columns with type 'timestamp without time zone'");
      }

      return _sqlExpressionFactory.Convert(arguments[0], arguments[0].Type,
                                           _timestampTzMapping);
    }
  ...
  private SqlExpression? CreateIntervalExpression(SqlExpression intervalNum,
                                                  string datePart) {
    // Note: ideally we'd simply generate a KingbaseES interval expression, but
    // the .NET mapping of that is TimeSpan, which does not work for months,
    // years, etc. So we generate special fragments instead.
    if (intervalNum is SqlConstantExpression constantExpression) {
      ...
      return _sqlExpressionFactory.Fragment(FormattableString.Invariant(
          $"INTERVAL '{constantExpression.Value} {datePart}'"));
    }

    // For non-constants, we can't parameterize INTERVAL '1 days'. Instead, we
    // use CAST($1 || ' days' AS interval). Note that a make_interval() function
    // also exists, but accepts only int (for all fields except for seconds), so
    // we don't use it. Note: we instantiate SqlBinaryExpression manually rather
    // than via sqlExpressionFactory because of the non-standard Add expression
    // (concatenate int with text)
    return _sqlExpressionFactory.Convert(
        new SqlBinaryExpression(
            ExpressionType.Add,
            _sqlExpressionFactory.Convert(intervalNum, typeof(string),
                                          _textMapping),
            _sqlExpressionFactory.Constant(' ' + datePart, _textMapping),
            typeof(string), _textMapping),
        typeof(TimeSpan), _intervalMapping);
  }
}
