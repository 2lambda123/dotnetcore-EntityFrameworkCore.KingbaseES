...
public class KdbndpTimeSpanMemberTranslator : IMemberTranslator {
  ...
  /// </summary>
  public KdbndpTimeSpanMemberTranslator(
  ...
  public virtual SqlExpression? Translate(
    ...
    if (member.DeclaringType == typeof(TimeSpan) && instance is not null) {
      return member.Name switch {
        ...
        nameof(TimeSpan.TotalDays) =>
            TranslateDurationTotalMember(instance, 86400),
        nameof(TimeSpan.TotalHours) =>
            TranslateDurationTotalMember(instance, 3600),
        nameof(TimeSpan.TotalMinutes) =>
            TranslateDurationTotalMember(instance, 60),
        nameof(TimeSpan.TotalSeconds) => DatePart("epoch", instance),
        nameof(TimeSpan.TotalMilliseconds) =>
            TranslateDurationTotalMember(instance, 0.001),
    ...
    SqlExpression Floor(SqlExpression value) => _sqlExpressionFactory.Convert(
        _sqlExpressionFactory.Function(
            "floor", new[] { value }, nullable: true,
    ...
    SqlFunctionExpression DatePart(string part, SqlExpression value) =>
        _sqlExpressionFactory.Function(
            ...
            nullable: true, argumentsPropagateNullability: FalseTrueArray,
            returnType);

    SqlBinaryExpression TranslateDurationTotalMember(SqlExpression instance,
                                                     double divisor) =>
        _sqlExpressionFactory.Divide(DatePart("epoch", instance),
                                     _sqlExpressionFactory.Constant(divisor));
  }
}
