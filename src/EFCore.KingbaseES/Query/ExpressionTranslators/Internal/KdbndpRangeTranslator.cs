...
public class KdbndpRangeTranslator : IMethodCallTranslator, IMemberTranslator {
  ...
  private static readonly MethodInfo EnumerableAnyWithoutPredicate =
      typeof(Enumerable)
          ...
          .Single(mi => mi.Name == nameof(Enumerable.Any) &&
  ...
  public KdbndpRangeTranslator(
      ...
      KdbndpSqlExpressionFactory KdbndpSqlExpressionFactory, IModel model,
  ...
  public virtual SqlExpression? Translate(
    ...
    // Any() over multirange -> NOT isempty(). KdbndpRange<T> has IsEmpty which
    // is translated below.
    if (_supportsMultiranges && method.IsGenericMethod &&
        method.GetGenericMethodDefinition() == EnumerableAnyWithoutPredicate) {
      return _sqlExpressionFactory.Not(_sqlExpressionFactory.Function(
          "isempty", new[] { arguments[0] }, nullable: true,
          argumentsPropagateNullability: TrueArrays[1], typeof(bool)));
    }

    if (method.DeclaringType != typeof(KdbndpRangeDbFunctionsExtensions) &&
        (method.DeclaringType !=
             typeof(KdbndpMultirangeDbFunctionsExtensions) ||
    ...
    if (method.Name == nameof(KdbndpRangeDbFunctionsExtensions.Merge)) {
      if (method.DeclaringType == typeof(KdbndpRangeDbFunctionsExtensions)) {
        ...
        return _sqlExpressionFactory.Function(
            "range_merge",
            new[] { _sqlExpressionFactory.ApplyTypeMapping(arguments[0],
                                                           inferredMapping),
                    _sqlExpressionFactory.ApplyTypeMapping(arguments[1],
                                                           inferredMapping) },
            nullable: true, argumentsPropagateNullability: TrueArrays[2],
            method.ReturnType, inferredMapping);
      }
    ...
    return method.Name switch {
      ...
      nameof(KdbndpRangeDbFunctionsExtensions.IsStrictlyLeftOf) =>
          _sqlExpressionFactory.MakePostgresBinary(
              PgExpressionType.RangeIsStrictlyLeftOf, arguments[0],
              arguments[1]),
      nameof(KdbndpRangeDbFunctionsExtensions.IsStrictlyRightOf) =>
          _sqlExpressionFactory.MakePostgresBinary(
              PgExpressionType.RangeIsStrictlyRightOf, arguments[0],
              arguments[1]),
      nameof(KdbndpRangeDbFunctionsExtensions.DoesNotExtendRightOf) =>
          _sqlExpressionFactory.MakePostgresBinary(
              PgExpressionType.RangeDoesNotExtendRightOf, arguments[0],
              arguments[1]),
      nameof(KdbndpRangeDbFunctionsExtensions.DoesNotExtendLeftOf) =>
          _sqlExpressionFactory.MakePostgresBinary(
              PgExpressionType.RangeDoesNotExtendLeftOf, arguments[0],
              arguments[1]),
      nameof(KdbndpRangeDbFunctionsExtensions.IsAdjacentTo) =>
          _sqlExpressionFactory.MakePostgresBinary(
              PgExpressionType.RangeIsAdjacentTo, arguments[0], arguments[1]),
      nameof(KdbndpRangeDbFunctionsExtensions.Union) =>
          _sqlExpressionFactory.MakePostgresBinary(PgExpressionType.RangeUnion,
                                                   arguments[0], arguments[1]),
      nameof(KdbndpRangeDbFunctionsExtensions.Intersect) =>
          _sqlExpressionFactory.MakePostgresBinary(
              PgExpressionType.RangeIntersect, arguments[0], arguments[1]),
      nameof(KdbndpRangeDbFunctionsExtensions.Except) =>
          _sqlExpressionFactory.MakePostgresBinary(PgExpressionType.RangeExcept,
                                                   arguments[0], arguments[1]),
  ...
  public virtual SqlExpression? Translate(
    ...
    if (type is null || !type.IsGenericType ||
        type.GetGenericTypeDefinition() != typeof(KdbndpRange<>)) {
      return null;
    }

    if (member.Name is nameof(KdbndpRange<int>.LowerBound)
            or nameof(KdbndpRange<int>.UpperBound)) {
      return null;
    }

    return member.Name switch {
      nameof(KdbndpRange<int>.IsEmpty) =>
          SingleArgBoolFunction("isempty", instance!),
      nameof(KdbndpRange<int>.LowerBoundIsInclusive) =>
          SingleArgBoolFunction("lower_inc", instance!),
      nameof(KdbndpRange<int>.UpperBoundIsInclusive) =>
          SingleArgBoolFunction("upper_inc", instance!),
      nameof(KdbndpRange<int>.LowerBoundInfinite) =>
          SingleArgBoolFunction("lower_inf", instance!),
      nameof(KdbndpRange<int>.UpperBoundInfinite) =>
          SingleArgBoolFunction("upper_inf", instance!),

      _ => null
    };

    SqlFunctionExpression SingleArgBoolFunction(string name,
                                                SqlExpression argument) =>
        _sqlExpressionFactory.Function(
            name, new[] { argument }, nullable: true,
            argumentsPropagateNullability: TrueArrays[1], typeof(bool));
  }
}
