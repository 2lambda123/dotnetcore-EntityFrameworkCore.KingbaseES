...
/// <summary>
///     Translates method and property calls on arrays/lists into their
///     corresponding KingbaseES operations.
/// </summary>
/// <remarks>
...
#region Methods
  ...
  private static readonly MethodInfo Enumerable_SequenceEqual =
      typeof(Enumerable)
          ...
          .GetMethods(BindingFlags.Public | BindingFlags.Static |
                      BindingFlags.DeclaredOnly)
          .Single(m => m.Name == nameof(Enumerable.SequenceEqual) &&
                       m.GetParameters().Length == 2);
  ...
  /// </summary>
  public KdbndpArrayMethodTranslator(
  ...
  public virtual SqlExpression? Translate(
    ...
    // During preprocessing, ArrayIndex and List[] get normalized to ElementAt;
    // so we handle indexing into array/list here
    if (method.IsClosedFormOf(Enumerable_ElementAt)) {
      // Indexing over bytea is special, we have to use function rather than
      // subscript
      if (arguments[0].TypeMapping is KdbndpByteArrayTypeMapping) {
        return _sqlExpressionFactory.Function(
            "get_byte", new[] { arguments[0], arguments[1] }, nullable: true,
            argumentsPropagateNullability: TrueArrays[2], typeof(byte));
      }
    ...
    if (instance is not null && instance.Type.IsGenericList() &&
        !IsMappedToNonArray(instance)) {
      return TranslateCommon(instance, arguments);
    }

    // Translate extension methods over array or List
    if (instance is null && arguments.Count > 0 &&
        arguments[0].Type.IsArrayOrGenericList() &&
        !IsMappedToNonArray(arguments[0])) {
    ...
    // The array/list CLR type may be mapped to a non-array database type (e.g.
    // byte[] to bytea, or just value converters) - we don't want to translate
    // for those cases.
    static bool IsMappedToNonArray(SqlExpression arrayOrList) =>
        arrayOrList.TypeMapping is {}
    and not(KdbndpArrayTypeMapping or KdbndpJsonTypeMapping);

#pragma warning disable CS8321
    SqlExpression ? TranslateCommon(SqlExpression arrayOrList,
                                    IReadOnlyList<SqlExpression> arguments)
#pragma warning restore CS8321
    {
      if (method.IsClosedFormOf(Array_IndexOf1) ||
        ...
        var (item, array) =
            _sqlExpressionFactory.ApplyTypeMappingsOnItemAndArray(arguments[0],
                                                                  arrayOrList);

        return _sqlExpressionFactory.Coalesce(
            _sqlExpressionFactory.Subtract(
                _sqlExpressionFactory.Function(
                    "array_position", new[] { array, item }, nullable: true,
                    TrueArrays[2], arrayOrList.Type),
      ...
      if (method.IsClosedFormOf(Array_IndexOf2) ||
        ...
        return _sqlExpressionFactory.Coalesce(
            _sqlExpressionFactory.Subtract(
                _sqlExpressionFactory.Function(
                    "array_position", new[] { array, item, startIndex },
                    nullable: true, TrueArrays[3], arrayOrList.Type),
                _sqlExpressionFactory.Constant(1)),
            _sqlExpressionFactory.Constant(-1));
      }
