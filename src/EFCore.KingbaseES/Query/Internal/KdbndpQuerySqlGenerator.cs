...
public class KdbndpQuerySqlGenerator : QuerySqlGenerator {
  ...
  /// <summary>
  ///     The backend version to target. If null, it means the user hasn't set a
  ///     compatibility version, and the latest should be targeted.
  /// </summary>
  private readonly Version _postgresVersion;

  /// <inheritdoc />
  public KdbndpQuerySqlGenerator(QuerySqlGeneratorDependencies dependencies,
  ...
  VisitExtension(Expression extensionExpression) => extensionExpression switch {
    ...
    PgTableValuedFunctionExpression e =>
        VisitPgTableValuedFunctionExpression(e),
  ...
  GenerateLimitOffset(SelectExpression selectExpression) {
  ...
  /// <inheritdoc />
  protected override
      string GetOperator(SqlBinaryExpression e) => e.OperatorType switch {
        // KingbaseES has a special string concatenation operator: ||
        // We switch to it if the expression itself has type string, or if one
        // of the sides has a string type mapping. Same for full-text search's
        // TsVector, arrays.
        ExpressionType.Add when e.Type == typeof(string) ||
            ...
            e.Left.TypeMapping is KdbndpArrayTypeMapping &&e.Right
                .TypeMapping is KdbndpArrayTypeMapping => " || ",
  ...
  /// </summary>
  protected override
      Expression VisitCrossApply(CrossApplyExpression crossApplyExpression) {
    Sql.Append("JOIN LATERAL ");

    if (crossApplyExpression.Table is TableExpression table) {
      // KingbaseES doesn't support LATERAL JOIN over table, and it doesn't
      // really make sense to do it - but EF Core will sometimes generate that.
      // #1560
  ...
  /// </summary>
  protected override
      Expression VisitOuterApply(OuterApplyExpression outerApplyExpression) {
  ...
  protected override Expression VisitSqlBinary(SqlBinaryExpression binary) {
    ...
    case ExpressionType.Add: {
      ...
      // KingbaseES 9.4 and below has some weird operator precedence fixed
      // in 9.5 and described here:
  ...
  // NonQueryConvertingExpressionVisitor converts the relational
  // DeleteExpression to PostgresDeleteExpression, so we should never get here
  /// <summary>
  ///     This is an internal API that supports the Entity Framework Core
  ///     infrastructure and not subject to the same compatibility standards as
  ///     public APIs. It may be changed or removed without notice in any
  ...
  /// </summary>
  protected override
      Expression VisitDelete(DeleteExpression deleteExpression) =>
  ...
  VisitPgDelete(PgDeleteExpression pgDeleteExpression) {
    Sql.Append("DELETE FROM ");
    Visit(pgDeleteExpression.Table);

    if (pgDeleteExpression.FromItems.Count > 0) {
      Sql.AppendLine().Append("USING ");
      GenerateList(pgDeleteExpression.FromItems, t => Visit(t),
                   sql => sql.Append(", "));
    }
  ...
  protected override Expression VisitUpdate(UpdateExpression updateExpression) {
    ...
    if (selectExpression.Offset == null && selectExpression.Limit == null &&
      ...
      using (Sql.Indent()) {
        foreach (var columnValueSetter in updateExpression.ColumnValueSetters
                     .Skip(1)) {
          Sql.AppendLine(",");
          Sql.Append(
      ...
      if (selectExpression.Tables.Count > 1) {
        ...
        for (var i = 0; i < selectExpression.Tables.Count; i++) {
          ...
          if (ReferenceEquals(updateExpression.Table,
                              joinExpression?.Table ?? table)) {
            LiftPredicate(table);
            continue;
          }

          visitor ??= new OuterReferenceFindingExpressionVisitor(
              updateExpression.Table);

          // KingbaseES doesn't support referencing the main update table from
          // anywhere except for the UPDATE WHERE clause. This specifically
          // makes it impossible to have joins which reference the main table in
          // their predicate (ON ...). Because of this, we detect all such inner
          // joins and lift their predicates to the main WHERE clause (where a
          // reference to the main table is allowed), producing UPDATE ... FROM
          // x, y WHERE y.foreign_key = x.id instead of INNER JOIN ... ON.
          if (firstTable) {
            LiftPredicate(table);
            table = joinExpression?.Table ?? table;
          } else if (joinExpression is InnerJoinExpression
                         innerJoinExpression &&
                     visitor.ContainsReferenceToMainTable(
                         innerJoinExpression.JoinPredicate)) {
            LiftPredicate(innerJoinExpression);

            Sql.AppendLine(",");
            using (Sql.Indent()) { Visit(innerJoinExpression.Table); }
          ...
          void LiftPredicate(TableExpressionBase joinTable) {
            if (joinTable is PredicateJoinExpressionBase
                    predicateJoinExpression) {
              Check.DebugAssert(joinExpression is not LeftJoinExpression,
                                "Cannot lift predicate for left join");
              ...
              predicate =
                  predicate == null
                      ...
                      : new SqlBinaryExpression(
                            ...
                            predicateJoinExpression.JoinPredicate, predicate,
                            typeof(bool), predicate.TypeMapping);
            }
          }
        }
      }
  ...
  /// </summary>
  protected virtual Expression
  VisitNewArray(PgNewArrayExpression pgNewArrayExpression) {
    ...
    Sql.Append("]::").Append(pgNewArrayExpression.TypeMapping.StoreType);
  ...
  /// </summary>
  protected virtual Expression
  VisitPgBinary(PgBinaryExpression binaryExpression) {
    Check.NotNull(binaryExpression, nameof(binaryExpression));

    var requiresParentheses =
        RequiresParentheses(binaryExpression, binaryExpression.Left);
    ...
    Sql.Append(" ")
        .Append(binaryExpression.OperatorType switch {
          ...
          PgExpressionType.LTreeMatches when binaryExpression.Right.TypeMapping
                      .StoreType == "lquery" ||
              binaryExpression.Right.TypeMapping is KdbndpArrayTypeMapping {
          ...
          PgExpressionType.LTreeFirstMatches when binaryExpression.Right
                  .TypeMapping.StoreType == "lquery" => "?~",
          PgExpressionType.LTreeFirstMatches when binaryExpression.Right
                  .TypeMapping.StoreType == "ltxtquery" => "?@",
    ...
    requiresParentheses =
        RequiresParentheses(binaryExpression, binaryExpression.Right);
  ...
  /// </summary>
  protected override
      Expression VisitSqlUnary(SqlUnaryExpression sqlUnaryExpression) {
    ...
    case ExpressionType.Convert: {
      // KingbaseES supports the standard CAST(x AS y), but also a lighter x::y
      // which we use where there's no precedence issues
      switch (sqlUnaryExpression.Operand) {
      case SqlConstantExpression:
      case SqlParameterExpression:
      case SqlUnaryExpression { OperatorType:
    ...
    case ScalarSubqueryExpression:
      var storeType = sqlUnaryExpression.TypeMapping.StoreType switch {
        "integer" => "int", "timestamp with time zone" => "timestamptz",
        "timestamp without time zone" => "timestamp", var s => s
      };
  ...
  case ExpressionType.Not when sqlUnaryExpression.Operand.TypeMapping.ClrType ==
          typeof(IPAddress) ||
      sqlUnaryExpression.Operand.TypeMapping.ClrType ==
          typeof((IPAddress, int)) ||
...
/// </summary>
protected override void
GenerateSetOperationOperand(SetOperationBase setOperation,
                            SelectExpression operand) {
  // KingbaseES allows ORDER BY and LIMIT in set operation operands, but
  // requires parentheses
  if (operand.Orderings.Count > 0 || operand.Limit is not null) {
    Sql.AppendLine("(");
    using (Sql.Indent()) { Visit(operand); }
...
/// </summary>
protected override
    Expression VisitCollate(CollateExpression collateExpression) {
  Check.NotNull(collateExpression, nameof(collateExpression));

  Visit(collateExpression.Operand);

  // In PG, collation names are regular identifiers which need to be quoted for
  // case-sensitivity.
  Sql.Append(" COLLATE ")
      .Append(
          _sqlGenerationHelper.DelimitIdentifier(collateExpression.Collation));
...
TryGenerateWithoutWrappingSelect(SelectExpression selectExpression)
    // KingbaseES supports VALUES as a top-level statement - and directly under
    // set operations. However, when on the left side of a set operation, we
    // need the column coming out of VALUES to be named, so we need the wrapping
    // SELECT for that.
...
protected override void GenerateSetOperation(SetOperationBase setOperation) {
  ...
  Sql.AppendLine()
      .Append(setOperation switch {
        ExceptExpression => "EXCEPT", IntersectExpression => "INTERSECT",
  ...
  // For ValuesExpression, we can remove its wrapping SelectExpression but only
  // if on the right side of a set operation, since on the left side we need the
  // column name to be specified.
...
protected override Expression VisitValues(ValuesExpression valuesExpression) {
  ...
  // KingbaseES VALUES supports setting the projects column names: FROM (VALUES
  // (1), (2)) AS v(foo)
  Sql.Append("(");
  ...
  for (var i = 0; i < valuesExpression.ColumnNames.Count; i++) {
    ...
    Sql.Append(_sqlGenerationHelper.DelimitIdentifier(
        valuesExpression.ColumnNames[i]));
  }
...
protected override void GenerateValues(ValuesExpression valuesExpression) {
  // KingbaseES supports providing the names of columns projected out of VALUES:
  // (VALUES (1, 3), (2, 4)) AS x(a, b). But since other databases sometimes
  // don't, the default relational implementation is complex, involving a SELECT
  // for the first row and a UNION All on the rest. Override to do the nice
  // simple thing.
...
protected virtual Expression VisitArrayAll(PgAllExpression expression) {
  ...
  Sql.Append(" ")
      .Append(expression.OperatorType switch {
        PgAllOperatorType.Like => "LIKE", PgAllOperatorType.ILike => "ILIKE",
        _ => throw new ArgumentOutOfRangeException(
            $"Unhandled operator type: {expression.OperatorType}")
      })
      .Append(" ALL (");
...
/// </summary>
protected virtual Expression
VisitArrayIndex(PgArrayIndexExpression expression) {
...
/// </summary>
protected virtual Expression
VisitArraySlice(PgArraySliceExpression expression) {
  var requiresParentheses = RequiresParentheses(expression, expression.Array);
...
/// </remarks>
protected virtual Expression
VisitRegexMatch(PgRegexMatchExpression expression) {
  ...
  var constantPattern =
      (expression.Pattern as SqlConstantExpression)?.Value as string;
  ...
  if (options.HasFlag(RegexOptions.Multiline)) {
    ...
    // In .NET's default mode, . doesn't match newlines but in KingbaseES it
    // does.
    Sql.Append("p");
  }
...
/// </summary>
protected virtual Expression
VisitRowValue(PgRowValueExpression rowValueExpression) {
  Sql.Append("(");
...
/// </summary>
protected override
    Expression VisitJsonScalar(JsonScalarExpression jsonScalarExpression) {
  ...
  // This case is for when a nested JSON entity is being accessed. We want the
  // json/jsonb fragment in this case (not text), so we can perform further JSON
  // operations on it.
  ...
  // bytea requires special handling, since we encode the binary data as base64
  // inside the JSON, but that requires a special conversion function to be
  // extracted out to a PG bytea.
  ...
  // Arrays require special handling; we cannot simply cast a JSON array (as
  // text) to a PG array ([1,2,3] isn't a valid PG array representation). We use
  // jsonb_array_elements_text to extract the array elements as a set, cast them
  // to their PG element type and then build an array from that.
  case KdbndpArrayTypeMapping arrayMapping:
    Sql.Append("(ARRAY(SELECT CAST(element AS ")
        .Append(arrayMapping.ElementTypeMapping.StoreType)
  ...
  void GenerateJsonPath(bool returnsText) => this.GenerateJsonPath(
      jsonScalarExpression.Json, returnsText: returnsText,
      jsonScalarExpression.Path
          .Select(s => s switch {
            { PropertyName : string propertyName } => new SqlConstantExpression(
...
/// </returns>
protected virtual Expression
VisitJsonPathTraversal(PgJsonTraversalExpression expression) {
  GenerateJsonPath(expression.Expression, expression.ReturnsText,
                   expression.Path);
  return expression;
}

private void GenerateJsonPath(SqlExpression expression, bool returnsText,
  ...
  // Use simplified array literal syntax if all path components are constants
  // for cleaner SQL
  if (path.All(p => p is SqlConstantExpression)) {
    Sql.Append("'{")
        .Append(string.Join(",",
                            path.Select(p => ((SqlConstantExpression)p).Value)))
        .Append("}'");
  } else {
    Sql.Append("ARRAY[");
...
///     to a new Entity Framework Core release.
/// </summary>
protected virtual Expression VisitPgTableValuedFunctionExpression(
    PgTableValuedFunctionExpression tableValuedFunctionExpression) {
  // PostgresTableValuedFunctionExpression extends the standard
  // TableValuedFunctionExpression, adding the possibility to specify column
  // names and types at the end, as well as an optional WITH ORDINALITY to
  // project an index out.

  // Note that KingbaseES doesn't support specifying both a column definition
  // (with type) *and* WITH ORDINALITY; but it does allow wrapping the function
  // invocation and column definition inside ROWS FROM, and placing the table
  // alias and WITH ORDINALITY outside. We take care of that here.
  if (tableValuedFunctionExpression
      is { WithOrdinality : true, ColumnInfos : {} columnInfos } &&
    ...
    Sql.Append(") WITH ORDINALITY AS ")
        .Append(_sqlGenerationHelper.DelimitIdentifier(
  ...
  void GenerateColumnDefinition() {
    ...
    if (tableValuedFunctionExpression.ColumnInfos is[var singleColumnInfo]) {
      ...
      for (var i = 0; i < tableValuedFunctionExpression.ColumnInfos.Count;
           i++) {
    ...
    void GenerateColumnInfo(
        PgTableValuedFunctionExpression.ColumnInfo columnInfo) {
...
VisitUnknownBinary(PgUnknownBinaryExpression unknownBinaryExpression) {
  ...
  var requiresParentheses = RequiresParentheses(unknownBinaryExpression,
                                                unknownBinaryExpression.Left);
  ...
  Sql.Append(" ").Append(unknownBinaryExpression.Operator).Append(" ");

  requiresParentheses = RequiresParentheses(unknownBinaryExpression,
                                            unknownBinaryExpression.Right);
...
protected virtual Expression VisitPgFunction(PgFunctionExpression e) {
  ...
  if (e.IsBuiltIn) {
    ...
    if (!string.IsNullOrEmpty(e.Schema)) {
      Sql.Append(_sqlGenerationHelper.DelimitIdentifier(e.Schema)).Append(".");
    }

    // TODO: Quote user-defined function names with upper-case (also for regular
  ...
  for (var i = 0; i < e.Arguments.Count; i++) {
    if (i < e.ArgumentNames.Count && e.ArgumentNames[i] is {} argumentName) {
      Sql.Append(argumentName).Append(" => ");
    }

    Visit(e.Arguments[i]);

    if (i < e.Arguments.Count - 1) {
      Sql.Append(i < e.ArgumentSeparators.Count &&
                         e.ArgumentSeparators[i] is not null
...
protected override bool RequiresParentheses(SqlExpression outerExpression,
  ...
  case SqlUnaryExpression innerUnary when outerExpression is SqlUnaryExpression
          outerUnary &&
      (innerUnary.OperatorType is ExpressionType.Negate ||
       innerUnary.OperatorType is ExpressionType.Not &&
           innerUnary.Type != typeof(bool)) &&
      (outerUnary.OperatorType is ExpressionType.Negate ||
       outerUnary.OperatorType is ExpressionType.Not &&
           outerUnary.Type != typeof(bool)):
    return true;
  ...
  case PgBinaryExpression innerBinary: {
    ...
    if (TryGetOperatorInfo(outerExpression, out var outerPrecedence,
      ...
      return outerPrecedence.CompareTo(innerPrecedence) switch {
        ...
        // If both operators have the same precedence, add parentheses unless
        // they're the same operator, and that operator is associative (e.g. a +
        // b + c)
        0 => outerExpression is not PgBinaryExpression outerBinary ||
             outerBinary.OperatorType != innerBinary.OperatorType ||
             !isOuterAssociative
             // Arithmetic operators on floating points aren't associative,
...
///     to a new Entity Framework Core release.
/// </summary>
protected override bool TryGetOperatorInfo(SqlExpression expression,
                                           out int precedence,
                                           out bool isAssociative) {
  // See
  // https://www.KingbaseES.org/docs/current/sql-syntax-lexical.html#SQL-PRECEDENCE
  (precedence, isAssociative) = expression switch {
    // TODO: Exponent => 1300

    SqlBinaryExpression
        sqlBinaryExpression => sqlBinaryExpression.OperatorType switch {
          ...
          ExpressionType.And when sqlBinaryExpression.Type != typeof(bool) =>
              (1000, true),
          ExpressionType.Or when sqlBinaryExpression.Type != typeof(bool) =>
              (1000, true),
        ...
        // There's an "any other operator" category in the PG operator
        // precedence table, we assign that a numeric value of 1000.
        // TODO: Some operators here may be associative
        PgBinaryExpression => (1000, false),

        CollateExpression => (1000, false),
        AtTimeZoneExpression => (1000, false), InExpression => (900, false),
        PgJsonTraversalExpression => (1000, false),
        PgArrayIndexExpression => (1500, false),
        PgAllExpression or PgAnyExpression => (800, false),
        LikeExpression or PgILikeExpression or PgRegexMatchExpression =>
            (900, false),
...
private void
GenerateList<T>(IReadOnlyList<T> items, Action<T> generationAction,
