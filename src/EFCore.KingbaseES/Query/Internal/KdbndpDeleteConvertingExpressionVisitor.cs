...
/// <summary>
///     Converts the relational <see cref="NonQueryExpression" /> into a
///     PG-specific <see cref="PgDeleteExpression" />, which precisely models a
///     DELETE statement in KingbaseES. This is done to handle the PG-specific
///     USING syntax for table joining.
/// </summary>
public class KdbndpDeleteConvertingExpressionVisitor : ExpressionVisitor {
  ...
  /// </summary>
  public virtual Expression Process(Expression node) => node switch {
  ...
  protected virtual Expression VisitDelete(DeleteExpression deleteExpression) {
    ...
    // The SelectExpression also contains the target table being modified (same
    // as deleteExpression.Table). If it has additional inner joins, use the
    // KingbaseES-specific USING syntax to express the join. Note that the
    // non-join TableExpression isn't necessary the target table - through
    // projection the last table being joined may be the one being modified.
    ...
    innerJoinExpression:
      ...
      joinPredicates =
          joinPredicates is null
              ? innerJoinExpression.JoinPredicate
              : new SqlBinaryExpression(
                    ExpressionType.AndAlso, joinPredicates,
                    innerJoinExpression.JoinPredicate, typeof(bool),
  ...
  var predicate = (joinPredicates, selectExpression.Predicate) switch {
    ...
    (not null, null) => joinPredicates, (null, null) => null,
    (not null, not null) => new SqlBinaryExpression(
        ExpressionType.AndAlso, joinPredicates, selectExpression.Predicate,
        typeof(bool), joinPredicates.TypeMapping)
  };

  return new PgDeleteExpression(deleteExpression.Table, fromItems, predicate,
