...
using Kdbndp.EntityFrameworkCore.KingbaseES.Query.ExpressionTranslators
    .Internal;
...
/// </summary>
public class KdbndpSqlTranslatingExpressionVisitor
  ...
  private static readonly ConstructorInfo DateTimeCtor2 =
      typeof(DateTime).GetConstructor(new[] { typeof(int), typeof(int),
  ...
  private static readonly ConstructorInfo DateTimeCtor3 =
      typeof(DateTime).GetConstructor(
          new[] { typeof(int), typeof(int), typeof(int), typeof(int),
                  typeof(int), typeof(int), typeof(DateTimeKind) })!;
  ...
  private static readonly MethodInfo StringStartsWithMethod =
      typeof(string).GetRuntimeMethod(nameof(string.StartsWith),
                                      new[] { typeof(string) })!;

  private static readonly MethodInfo StringEndsWithMethod =
      typeof(string).GetRuntimeMethod(nameof(string.EndsWith),
                                      new[] { typeof(string) })!;

  private static readonly MethodInfo StringContainsMethod =
      typeof(string).GetRuntimeMethod(nameof(string.Contains),
                                      new[] { typeof(string) })!;

  private static readonly MethodInfo EscapeLikePatternParameterMethod =
      typeof(KdbndpSqlTranslatingExpressionVisitor)
          .GetTypeInfo()
          .GetDeclaredMethod(nameof(ConstructLikePatternParameter))!;

  // Note: This is the KingbaseES default and does not need to be explicitly
  // specified
  ...
  public KdbndpSqlTranslatingExpressionVisitor(
    ...
    _sqlExpressionFactory =
        (KdbndpSqlExpressionFactory)dependencies.SqlExpressionFactory;
    _jsonPocoTranslator =
        ((KdbndpMemberTranslatorProvider)Dependencies.MemberTranslatorProvider)
            .JsonPocoTranslator;
  ...
  protected override Expression VisitUnary(UnaryExpression unaryExpression) {
    ...
    case ExpressionType.ArrayLength:
      if (TranslationFailed(unaryExpression.Operand,
      ...
      // For byte[] mapped to an actual PG array (smallint[]), that's a
      // primitive collection, and ArrayLength gets transformed to Count() which
      // gets translated to cardinality() as usual in
      // KdbndpQueryableMethodTranslatingExpressionVisitor.
      if (sqlOperand!.Type == typeof(byte[]) &&
          sqlOperand.TypeMapping is KdbndpByteArrayTypeMapping or null) {
        return _sqlExpressionFactory.Function(
            "length", new[] { sqlOperand }, nullable: true,
            argumentsPropagateNullability: TrueArrays[1], typeof(int));
      }

      // Attempt to translate Length on a JSON POCO array
      if (_jsonPocoTranslator.TranslateArrayLength(sqlOperand)
              is SqlExpression translation) {
        return translation;
      }

      // Note that Length over PG arrays (not within JSON) gets translated by
      // QueryableMethodTranslatingEV, since arrays are primitive collections
      break;

    // We have row value comparison methods such as EF.Functions.GreaterThan,
    // which accept two ValueTuples/Tuples. Since they accept ITuple parameters,
    // the arguments have a Convert node casting up from the concrete argument
    // to ITuple; this node causes translation failure in
    // RelationalSqlTranslatingExpressionVisitor, so unwrap here.
    case ExpressionType.Convert when unaryExpression.Type == typeof(ITuple) &&
  ...
  /// </summary>
  protected override
      Expression VisitNewArray(NewArrayExpression newArrayExpression) {
    if (base.VisitNewArray(newArrayExpression)
            is SqlExpression visitedNewArrayExpression) {
      return visitedNewArrayExpression;
    }

    if (newArrayExpression.NodeType == ExpressionType.NewArrayInit) {
      var visitedExpressions =
      ...
      return _sqlExpressionFactory.NewArray(visitedExpressions,
  ...
  protected override Expression VisitBinary(BinaryExpression binaryExpression) {
    ...
    case ExpressionType.Subtract when binaryExpression.Left.Type
                .UnwrapNullableType()
                .FullName == "NodaTime.LocalDate" &&
        binaryExpression.Right.Type.UnwrapNullableType().FullName ==
            "NodaTime.LocalDate": {
      if (TranslationFailed(
              binaryExpression.Left,
              Visit(TryRemoveImplicitConvert(binaryExpression.Left)),
              out var sqlLeft) ||
          TranslationFailed(
              binaryExpression.Right,
              Visit(TryRemoveImplicitConvert(binaryExpression.Right)),
              out var sqlRight)) {
        return QueryCompilationContext.NotTranslatedExpression;
      }

      var subtraction = _sqlExpressionFactory.MakeBinary(
          ExpressionType.Subtract, sqlLeft!, sqlRight!,
          _typeMappingSource.FindMapping(typeof(int)))!;

      return PgFunctionExpression.CreateWithNamedArguments(
          "make_interval", new[] { subtraction }, new[] { "days" },
          nullable: true, argumentsPropagateNullability: TrueArrays[1],
          builtIn: true,
          _nodaTimePeriodType ??=
              binaryExpression.Left.Type.Assembly.GetType("NodaTime.Period")!,
          typeMapping: null);

      // Note: many other date/time arithmetic operators are fully supported
      // as-is by KingbaseES - see KdbndpSqlExpressionFactory
    }

    case ExpressionType.ArrayIndex: {
      // During preprocessing, ArrayIndex and List[] get normalized to
      // ElementAt; see KdbndpArrayTranslator
      Check.DebugFail(
    ...
    // When an optional owned JSON entity is compared to null, we get WHERE (x
    // -> y) IS NULL. The -> operator (returning jsonb) is used rather than ->>
    // (returning text), since an entity type is being extracted, and further
    // JSON operations may need to be composed. However, when the value
    // extracted is a JSON null, a non-NULL jsonb value is returned, and
    // comparing that to relational NULL returns false. Pattern-match this and
    // force the use of ->> by changing the mapping to be a scalar rather than
    // an entity type.
    if (translation is SqlUnaryExpression {
          OperatorType : ExpressionType.Equal or ExpressionType.NotEqual,
          Operand : JsonScalarExpression {
            TypeMapping : KdbndpOwnedJsonTypeMapping
          } operand
        } unary) {
      return unary.Update(new JsonScalarExpression(
          operand.Json, operand.Path, operand.Type,
          _typeMappingSource.FindMapping("text"), operand.IsNullable));
    }
  ...
  protected override
    ...
    if (method == StringStartsWithMethod &&
        TryTranslateStartsEndsWithContains(
            ...
            StartsEndsWithContains.StartsWith, out var translation1)) {
      return translation1;
    }
    ...
    if (method == StringEndsWithMethod &&
        TryTranslateStartsEndsWithContains(
            methodCallExpression.Object!, methodCallExpression.Arguments[0],
            StartsEndsWithContains.EndsWith, out var translation2)) {
      return translation2;
    }
    ...
    if (method == StringContainsMethod &&
        TryTranslateStartsEndsWithContains(
            methodCallExpression.Object!, methodCallExpression.Arguments[0],
            StartsEndsWithContains.Contains, out var translation3)) {
      return translation3;
    }
  ...
  protected override Expression VisitNew(NewExpression newExpression) {
    ...
    if (visitedNewExpression !=
        QueryCompilationContext.NotTranslatedExpression) {
      return visitedNewExpression;
    }

    // We translate new ValueTuple<T1, T2...>(x, y...) to a SQL row value
    // expression: (x, y). This is notably done to support row value
    // comparisons: WHERE (x, y) > (3, 4) (see e.g.
    // KdbndpDbFunctionsExtensions.GreaterThan)
    ...
    if (newExpression.Constructor?.DeclaringType == typeof(DateTime)) {
      if (newExpression.Constructor == DateTimeCtor1) {
        return TryTranslateArguments(out var sqlArguments)
                   ? _sqlExpressionFactory.Function(
                         ...
                         TrueArrays[3], typeof(DateTime), _timestampMapping)
                   : QueryCompilationContext.NotTranslatedExpression;
      }
      ...
      if (newExpression.Constructor == DateTimeCtor2) {
        ...
        // DateTime's second component is an int, but KingbaseES's
        // MAKE_TIMESTAMP accepts a double precision
        sqlArguments[5] =
            _sqlExpressionFactory.Convert(sqlArguments[5], typeof(double));

        return _sqlExpressionFactory.Function(
            "make_timestamp", sqlArguments, nullable: true, TrueArrays[6],
            typeof(DateTime), _timestampMapping);
      }

      if (newExpression.Constructor == DateTimeCtor3 &&
          newExpression.Arguments[6] is
          ConstantExpression { Value : DateTimeKind kind }) {
        if (!TryTranslateArguments(out var sqlArguments)) {
          return QueryCompilationContext.NotTranslatedExpression;
        }

        // DateTime's second component is an int, but KingbaseES's
        // make_timestamp/make_timestamptz accepts a double precision. Also chop
        // off the last Kind argument which does not get sent to KingbaseES
        ...
        return kind == DateTimeKind.Utc
                   ? _sqlExpressionFactory.Function(
                         "make_timestamptz", rewrittenArguments, nullable: true,
                         TrueArrays[8], typeof(DateTime), _timestampTzMapping)
                   : _sqlExpressionFactory.Function(
                         "make_timestamp", rewrittenArguments, nullable: true,
                         TrueArrays[7], typeof(DateTime), _timestampMapping);
      }
    }
    ...
    if (newExpression.Constructor == DateOnlyCtor) {
      return TryTranslateArguments(out var sqlArguments)
                 ? _sqlExpressionFactory.Function("make_date", sqlArguments,
                                                  nullable: true, TrueArrays[3],
                                                  typeof(DateOnly))
                 : QueryCompilationContext.NotTranslatedExpression;
    }
...
#region StartsWith / EndsWith / Contains
  ...
  private bool TryTranslateStartsEndsWithContains(
      Expression instance, Expression pattern,
    ...
    var stringTypeMapping = ExpressionExtensions.InferTypeMapping(
        translatedInstance, translatedPattern);

    translatedInstance = _sqlExpressionFactory.ApplyTypeMapping(
        translatedInstance, stringTypeMapping);
    translatedPattern = _sqlExpressionFactory.ApplyTypeMapping(
        translatedPattern, stringTypeMapping);

    switch (translatedPattern) {
    case SqlConstantExpression patternConstant: {
      // The pattern is constant. Aside from null and empty string, we escape
      // all special characters (%, _, \) and send a simple LIKE
    ...
    case SqlParameterExpression patternParameter when patternParameter.Name
        .StartsWith(QueryCompilationContext.QueryParameterPrefix,
                    StringComparison.Ordinal): {
      // The pattern is a parameter, register a runtime parameter that will
      // contain the rewritten LIKE pattern, where all special characters have
      // been escaped.
      var lambda = Expression.Lambda(
          Expression.Call(EscapeLikePatternParameterMethod,
      ...
      var escapedPatternParameter =
          _queryCompilationContext.RegisterRuntimeParameter(
              patternParameter.Name + "_rewritten", lambda);
    ...
    default:
      // The pattern is a column or a complex expression; the possible special
      // characters in the pattern cannot be escaped, preventing us from
      // translating to LIKE.
      switch (methodType) {
      // For StartsWith/EndsWith, use LEFT or RIGHT instead to extract substring
      // and compare: WHERE instance IS NOT NULL AND pattern IS NOT NULL AND
      // LEFT(instance, LEN(pattern)) = pattern This is less efficient than LIKE
      // (i.e. StartsWith does an index scan instead of seek), but we have no
      // choice.
      case StartsEndsWithContains.StartsWith or StartsEndsWithContains.EndsWith:
            translation = _sqlExpressionFactory.Function(
                methodType is StartsEndsWithContains.StartsWith ? "left"
                                                                : "right",
                new[] { translatedInstance,
                        _sqlExpressionFactory.Function(
                            "length", new[] { translatedPattern },
                            nullable: true,
            ...
            // LEFT/RIGHT of a citext return a text, so for non-default text
            // mappings we apply an explicit cast.
            if (translatedInstance.TypeMapping is { StoreType : not "text" }) {
              translation = _sqlExpressionFactory.Convert(
                  translation, typeof(string), translatedInstance.TypeMapping);
            }

            // We compensate for the case where both the instance and the
            // pattern are null (null.StartsWith(null)); a simple equality would
            // yield true in that case, but we want false.
            translation = _sqlExpressionFactory.AndAlso(
                ...
                _sqlExpressionFactory.AndAlso(
                    ...
                    _sqlExpressionFactory.Equal(translation,
                                                translatedPattern)));

            break;

      // For Contains, just use strpos and check if the result is greater than
      // 0. Note that strpos returns 1 when the pattern is an empty string, just
      // like .NET Contains (so no need to compensate)
            ...
            translation = _sqlExpressionFactory.AndAlso(
                ...
                _sqlExpressionFactory.AndAlso(
                    ...
                    _sqlExpressionFactory.GreaterThan(
                        _sqlExpressionFactory.Function(
                            "strpos",
                            new[] { translatedInstance, translatedPattern },
                            nullable: true,
                            argumentsPropagateNullability: new[] { true, true },
                            typeof(int)),
    ...
    private static string
        ? ConstructLikePatternParameter(QueryContext queryContext,
    ...
    private enum StartsEndsWithContains { StartsWith, EndsWith, Contains }
            ...
            if (expression is UnaryExpression {
              ...
              if (innerType == convertedType ||
                  (convertedType == typeof(int) &&
                   (innerType == typeof(byte) || innerType == typeof(sbyte) ||
                    innerType == typeof(char) || innerType == typeof(short) ||
