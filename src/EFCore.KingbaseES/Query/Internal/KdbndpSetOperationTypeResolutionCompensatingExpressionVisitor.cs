...
/// </summary>
public class KdbndpSetOperationTypeResolutionCompensatingExpressionVisitor
    : ExpressionVisitor {
  ...
  VisitExtension(Expression extensionExpression) => extensionExpression switch {
    ...
    SetOperationBase setOperationExpression =>
        VisitSetOperation(setOperationExpression),
    SelectExpression selectExpression => VisitSelect(selectExpression),
    _ => base.VisitExtension(extensionExpression)
  };

  private Expression
  VisitSetOperation(SetOperationBase setOperationExpression) {
    switch (_state) {
    case State.Nothing:
  ...
  private Expression VisitSelect(SelectExpression selectExpression) {
    ...
    // Above we visited the tables, which may contain nested set operations - so
    // we retained our state. When visiting the below elements, reset to state
    // to properly handle nested unions inside e.g. the predicate.
    ...
    foreach (var item in selectExpression.Projection) {
      ...
      var updatedProjection =
          parentState == State.InNestedSetOperation &&
                  item.Expression is SqlConstantExpression { Value : null }
              ? item.Update(new SqlUnaryExpression(
                    ExpressionType.Convert, item.Expression,
                    item.Expression.Type, item.Expression.TypeMapping))
    ...
    // If we were in the InNestedSetOperation state, we've applied all explicit
    // type mappings when visiting the ProjectionExpressions above; change the
    // state to prevent unnecessarily continuing to compensate
    _state = parentState == State.InNestedSetOperation
                 ? State.AlreadyCompensated
                 : parentState;

    return changed ? selectExpression.Update(projections, tables, predicate,
                                             groupBy, havingExpression,
