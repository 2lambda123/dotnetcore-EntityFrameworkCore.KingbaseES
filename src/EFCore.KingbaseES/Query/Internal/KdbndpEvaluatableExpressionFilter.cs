  ...
  public override bool IsEvaluatableExpression(Expression expression,
    ...
    case MethodCallExpression methodCallExpression:
      ...
      if (method == TsQueryParse || method == TsVectorParse ||
          ...
          // Prevent evaluation of ValueTuple.Create, see NewExpression of
          // ITuple below
          || declaringType == typeof(ValueTuple) &&
    ...
    case NewExpression newExpression when newExpression.Type.IsAssignableTo(
        typeof(ITuple)):
      // We translate new ValueTuple<T1, T2...>(x, y...) to a SQL row value
      // expression: (x, y) (see
      // KdbndpSqlTranslatingExpressionVisitor.VisitNew). We must prevent
      // evaluation when the tuple contains only constants/parameters, since SQL
      // row values cannot be parameterized; we need to render them as
      // "literals" instead: WHERE (x, y) > (3, $1)
      return false;
    }
