...
/// <summary>
///     An expression that represents a KingbaseES-specific binary operation in
///     a SQL tree.
/// </summary>
public class PgBinaryExpression : SqlExpression {
  ...
  public PgBinaryExpression(PgExpressionType operatorType, SqlExpression left,
                            SqlExpression right, Type type,
  ...
  /// <param name="right">The <see cref="Right" /> property of the
  /// result.</param> <returns>This expression if no children changed, or an
  /// expression with the updated children.</returns>
  public virtual PgBinaryExpression Update(SqlExpression left,
                                           SqlExpression right) {
    Check.NotNull(left, nameof(left));
    Check.NotNull(right, nameof(right));

    return left != Left || right != Right
               ? new PgBinaryExpression(OperatorType, left, right, Type,
                                        TypeMapping)
               : this;
  }
  ...
  protected override void Print(ExpressionPrinter expressionPrinter) {
    ...
    expressionPrinter.Append(" ")
        .Append(OperatorType switch {
          ...
          PgExpressionType.LTreeMatches when Right.TypeMapping
          is { StoreType : "lquery" } or
          KdbndpArrayTypeMapping { ElementTypeMapping.StoreType : "lquery" } =>
              "~",
          PgExpressionType.LTreeMatches when Right.TypeMapping?.StoreType ==
              "ltxtquery" => "@",
          PgExpressionType.LTreeMatchesAny => "?",
          PgExpressionType.LTreeFirstAncestor => "?@>",
          PgExpressionType.LTreeFirstDescendent => "?<@",
          PgExpressionType.LTreeFirstMatches when
                  Right.TypeMapping?.StoreType == "lquery" => "?~",
          PgExpressionType.LTreeFirstMatches when
                  Right.TypeMapping?.StoreType == "ltxtquery" => "?@",
  ...
  public override bool
      Equals(object? obj) => obj is not null &&
