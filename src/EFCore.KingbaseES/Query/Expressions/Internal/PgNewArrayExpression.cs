...
public class PgNewArrayExpression : SqlExpression {
  /// <summary>
  ///     Creates a new instance of the <see cref="PgNewArrayExpression" />
  ///     class.
  /// </summary>
  /// <param name="expressions">The values to initialize the elements of the new
  /// array.</param> <param name="type">The <see cref="Type" /> of the
  /// expression.</param> <param name="typeMapping">The <see
  /// cref="RelationalTypeMapping" /> associated with the expression.</param>
  public PgNewArrayExpression(IReadOnlyList<SqlExpression> expressions,
    ...
    if (type.TryGetElementType(typeof(IEnumerable<>)) is null) {
      throw new ArgumentException(
          $"{nameof(PgNewArrayExpression)} must have an IEnumerable<T> type");
    }
  ...
  /// </summary>
  /// <param name="expressions">The values to initialize the elements of the new
  /// array.</param> <returns>This expression if no children changed, or an
  /// expression with the updated children.</returns>
  public virtual PgNewArrayExpression
  Update(IReadOnlyList<SqlExpression> expressions) {
    Check.NotNull(expressions, nameof(expressions));
