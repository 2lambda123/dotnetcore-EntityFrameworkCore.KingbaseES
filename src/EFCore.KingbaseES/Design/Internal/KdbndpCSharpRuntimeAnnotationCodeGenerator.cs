...
public class KdbndpCSharpRuntimeAnnotationCodeGenerator
  ...
  Create(CoreTypeMapping typeMapping,
         CSharpRuntimeAnnotationCodeGeneratorParameters parameters,
         ValueComparer? valueComparer = null,
         ValueComparer? keyValueComparer = null,
         ValueComparer? providerValueComparer = null) {
    var result = base.Create(typeMapping, parameters, valueComparer,
                             keyValueComparer, providerValueComparer);
    ...
    if (KdbndpDbTypeBasedDefaultInstance is not null) {
      ...
      if (KdbndpDbType != KdbndpDbTypeBasedDefaultInstance.KdbndpDbType) {
        ...
        mainBuilder.Append(nameof(KdbndpTypes))
            .Append(".")
...
#pragma warning disable CS0618 // KdbndpConnection.GlobalTypeMapper is obsolete
    case KdbndpEnumTypeMapping enumTypeMapping:
      if (enumTypeMapping.NameTranslator !=
          KdbndpConnection.GlobalTypeMapper.DefaultNameTranslator) {
...
#pragma warning restore CS0618
  ...
  /// <inheritdoc />
  public override void
  Generate(IRelationalModel model,
           CSharpRuntimeAnnotationCodeGeneratorParameters parameters) {
    if (!parameters.IsRuntime) {
      var annotations = parameters.Annotations;
      annotations.Remove(KdbndpAnnotationNames.DatabaseTemplate);
...
#pragma warning restore CS0618
  ...
  /// <inheritdoc />
  public override void
  Generate(IEntityType entityType,
           CSharpRuntimeAnnotationCodeGeneratorParameters parameters) {
    if (!parameters.IsRuntime) {
      var annotations = parameters.Annotations;
