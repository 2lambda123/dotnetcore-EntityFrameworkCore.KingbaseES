...
internal static class Program {
  static void Main(string[] args) {
    ...
    services.AddDbContext<BlogContext>(options => {
      options.UseKdbndp(
          @"host=localhost;port=54321;database=test;user id=system;password=123456;");
    });
    ...
    Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") +
                      "当前 Blogs 表中条目数：" + blogs.Count);
    if (blogs.Any()) {
      Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") +
                        "当前 Blogs 表中条目数大于1，进行清理...");
    ...
    Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") +
                      "变更 Blog 实体 值为I love EFCore too!");
